/**
 * @file    skipper_config.h
 * @brief   Skipper GCS PB2-I firmware — compile-time configuration constants.
 *
 ...
 *
 * All hardware and operational parameters for the Skipper GCS PocketBeagle 2
 ...
 *
 * Security note:
 *   TPM key slot assignments (SKIPPER_TPM_KEY_HANDLE) are persistent across
 *   reboots.  Changing the handle after initial provisioning requires
 *   explicit TPM NV clear.  See provisioning procedure in
 *   gcs/skipper/firmware/pb2i/PROVISIONING.md (to be created in Phase
 * Skipper-2).
 */

#ifndef SKIPPER_CONFIG_H
#define SKIPPER_CONFIG_H
...
#define SKIPPER_MAVLINK_SYS_ID             (255U)

/** MAVLink component ID — GCS master. */
#define SKIPPER_MAVLINK_COMP_ID            (190U)

/** Aircraft MAVLink system ID (Serenity UAV). */
#define SKIPPER_AIRCRAFT_SYS_ID            (1U)
...
#define SKIPPER_LORA_SPI_DEV               "/dev/spidev1.0"
 ...
 */
#define SKIPPER_LORA_TX_POWER_DBM          (20U)

/** LoRa spreading factor (SF7–SF12). */
#define SKIPPER_LORA_SPREADING_FACTOR      (10U)

/** LoRa bandwidth (Hz): 125000, 250000, or 500000. */
#define SKIPPER_LORA_BANDWIDTH_HZ          (125000U)
...
/** WiFi interface name. */
#define SKIPPER_WIFI_IFACE                 "wlan0"
 ...
 */
#define SKIPPER_WIFI_TX_POWER_MBM          (1700U)
...
#define SKIPPER_49MHZ_XCVR_UART_DEV        "/dev/ttyS5"

/** XCVR-49MHZ-2 UART baud rate (1200 Bd Bell 202 AFSK). */
#define SKIPPER_49MHZ_XCVR_BAUD            (1200U)
 ...
 * = Channel 1.
 */
#define SKIPPER_49MHZ_XCVR_DEFAULT_CHANNEL (0U)

/** GPIO chip for XCVR-49MHZ-2 PTT_N line. */
#define SKIPPER_49MHZ_XCVR_GPIO_CHIP       "/dev/gpiochip0"
 ...
 */
#define SKIPPER_GNSS_DEV_HOST              "/dev/gnss_gcs"

/** GNSS baud rate. */
#define SKIPPER_GNSS_BAUD_HOST             (115200U)

/* ---------------------------------------------------------------------------
 * Security
 * ---------------------------------------------------------------------------*/

/** TPM2 HMAC key persistent handle for Skipper's GCS node. */
#define SKIPPER_TPM_KEY_HANDLE             (0x81000001U)

/** MAVLink HMAC-SHA256 signing key length (bytes). */
#define SKIPPER_HMAC_KEY_LEN               (32U)
...
#define SKIPPER_HEARTBEAT_TIMEOUT_MS       (5000U)

#endif /* SKIPPER_CONFIG_H */
