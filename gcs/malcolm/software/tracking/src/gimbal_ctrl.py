"""
gimbal_ctrl.py — Malcolm GCS gimbal controller: sends servo commands to PB2-I.
...
the firmware (which enforces the same limits in mal_gimbal.c).
...
Usage:
    python3 gimbal_ctrl.py [--config malcolm_config.yaml]
"""
...
# ---------------------------------------------------------------------------
# Travel limits (mirrors mal_config.h — change both if limits change)
# ---------------------------------------------------------------------------
...
class GimbalController:
    ...
    def run(self) -> None:
        ...
        log.info(
            ...
            self._listen_port, self._pb2i_host, self._pb2i_port,
        )
    ...
    def _send_command(
        ...
        """Send a GIMBAL_TARGET command to Malcolm's PB2-I."""
        cmd = json.dumps({
            "cmd": "GIMBAL_TARGET",
            "az":  round(pan_deg,  2),
            "el":  round(tilt_deg, 2),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Malcolm GCS gimbal controller")
    parser.add_argument(
        "--config", "-c",
        type=Path,
