...
COMPONENTS = [
    ...
    ("fuselage/head_shell24_2mm_repaired.stl", "Head", FUSELAGE_COLOR),
    ...
    ("fuselage/dorsal_antenna_fin.stl", "Dorsal_Fin", DETAIL_COLOR),
...
def setup_lighting() -> None:
    ...
    def add_sun(
        name: str, direction: tuple, strength: float, color=(1.0, 0.98, 0.92)
    ) -> None:
...
def add_camera(
    name: str, location: tuple, target: tuple, lens: float = 85.0
) -> bpy.types.Object:
...
def main() -> None:
    ...
    print(
        f"[render] === Complete. {len(OVERVIEW_CAMERAS) + len(ISO_CAMERAS) + len(CLOSEUP_CAMERAS)} images → {OUT} ==="
    )
