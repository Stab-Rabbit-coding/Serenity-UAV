...
AFT_MOTOR_MOUNT_FROM_INTAKE = (
    108.0  # mm from Z=0 intake → EDF2 motor struts (98..143mm range)
)
FRONT_MOTOR_MOUNT_FROM_INTAKE = (
    35.0  # mm from Z=0 intake → EDF1 motor struts (22..72mm range)
)

# ── Thrust tube (50mm ID sleeve) constants ────────────────────────────────────
BORE_CARVE_R = 27.5  # mm — bore carve radius (EDF casing OD=55mm → R=27.5mm)
OLD_PETAL_FROM_INTAKE = (
    4.58 * 25.4
)  # mm — old fixed nozzle petal start (≈116.3mm); remove them
...
PIVOT_Z_FROM_INTAKE = (
    83.0  # mm — Rev O: pivot at nacelle CG, computed from mass breakdown above
)
CLEVIS_ARM_SLOT = (
    16.0  # mm — slot width between nacelle X faces; arm width must be ≤15mm
)
CLEVIS_EAR_THICK = 10.0  # mm — X-direction thickness of each pivot boss (MF104ZZ is 4mm wide + 1mm margin)
CLEVIS_EAR_OD = (
    26.0  # mm — boss outer diameter (MF104ZZ OD=10mm + 2×8mm CF-PETG housing wall)
)
CLEVIS_PIN_D = (
    8.0  # mm — MF104ZZ bearing ID (4mm) + clearance; hinge-pin = CF spar through-bore
)
...
THRUST_TUBE_WALL = (
    2.5  # mm — thrust tube wall thickness (OD = 50+5 = 55mm = EDF casing OD)
)
...
PINION_A_Y_OFFSET = (
    28.0  # mm — 28mm fore-aft from bore centre = R_sector(22) + R_pinion_A(6);
)
...
SHAFT_CONDUIT_ANGLE = (
    -0.5
)  # rad — angular position of conduit on bore (retained for legacy geometry)
SHAFT_CONDUIT_R = (
    28.0  # mm — Y offset of conduit from bore axis; matches PINION_A_Y_OFFSET
)
CROWN_PINION_Z_REF = (
    133.0  # mm — crown pinion shaft centre (1× ref); near nozzle ring bottom
)
...
def add_hub_ring(bm, cx, cy, r_out, r_in, z_bot, z_top, n_seg):
    ...
    bo = [
        bm.verts.new((cx + r_out * math.cos(t), cy + r_out * math.sin(t), z_bot))
        for t in a
    ]
    to = [
        bm.verts.new((cx + r_out * math.cos(t), cy + r_out * math.sin(t), z_top))
        for t in a
    ]
    bi = [
        bm.verts.new((cx + r_in * math.cos(t), cy + r_in * math.sin(t), z_bot))
        for t in a
    ]
    ti = [
        bm.verts.new((cx + r_in * math.cos(t), cy + r_in * math.sin(t), z_top))
        for t in a
    ]
...
def make_nozzle_ring(
    ...
    bot_o = [
        bm.verts.new((cx + outer_r * math.cos(a), cy + outer_r * math.sin(a), 0.0))
        for a in angles
    ]
    top_o = [
        bm.verts.new((cx + outer_r * math.cos(a), cy + outer_r * math.sin(a), axial_h))
        for a in angles
    ]
    bot_i = [
        bm.verts.new(
            (
                cx + inner_radius(a) * math.cos(a),
...
def make_housing_shell(name, cx, cy, z_bot, z_top, inner_r, wall_thickness, n_seg=64):
    ...
    bot_i = [
        bm.verts.new((cx + inner_r * math.cos(a), cy + inner_r * math.sin(a), z_bot))
        for a in angles
    ]
    top_i = [
        bm.verts.new((cx + inner_r * math.cos(a), cy + inner_r * math.sin(a), z_top))
        for a in angles
    ]
    bot_o = [
        bm.verts.new((cx + outer_r * math.cos(a), cy + outer_r * math.sin(a), z_bot))
        for a in angles
    ]
    top_o = [
        bm.verts.new((cx + outer_r * math.cos(a), cy + outer_r * math.sin(a), z_top))
        for a in angles
    ]
...
def make_motor_mount(
    name, cx, cy, z_center, inner_r, outer_r, thickness=3.0, axial_length=8.0, n_arms=4
):
...
def make_wire_guide(
    ...
    length = z_top - z_bot
    center = Vector((cx, cy, z_mid)) + Vector(
...
def make_thrust_tube(
    name, cx, cy, z_bot, z_top, inner_r=25.0, wall=THRUST_TUBE_WALL, n_seg=64
):
...
def make_inlet_bell(
    ...
    z_throat,
    r_throat=R_FIN_OUT,
    flare=INLET_BELL_FLARE,
    wall=THRUST_TUBE_WALL,
    n_seg=64,
    n_rings=16,
):
    ...
    for z in zs:
        ...
        rings_i.append(
            [
                bm.verts.new((cx + ri * math.cos(a), cy + ri * math.sin(a), z))
                for a in angles
            ]
        )
        rings_o.append(
            [
                bm.verts.new((cx + ro * math.cos(a), cy + ro * math.sin(a), z))
                for a in angles
            ]
        )
    ...
    for k in range(n_rings - 1):
        for i in range(n_seg):
            j = (i + 1) % n_seg
            add_face(
                bm, [rings_i[k][i], rings_i[k][j], rings_i[k + 1][j], rings_i[k + 1][i]]
            )  # inner wall
            add_face(
                bm, [rings_o[k][i], rings_o[k + 1][i], rings_o[k + 1][j], rings_o[k][j]]
            )  # outer wall

    for i in range(n_seg):
        j = (i + 1) % n_seg
        add_face(
            bm, [rings_o[0][i], rings_i[0][i], rings_i[0][j], rings_o[0][j]]
        )  # entry lip annulus
        add_face(
            bm, [rings_o[-1][i], rings_o[-1][j], rings_i[-1][j], rings_i[-1][i]]
        )  # throat annulus
...
def make_clevis_extension(
    name,
    bore_cx,
    bore_cy,
    z_center,
    arm_slot=CLEVIS_ARM_SLOT,
    ear_thick=CLEVIS_EAR_THICK,
    ear_od=CLEVIS_EAR_OD,
    y_from_bore=CLEVIS_Y_FROM_BORE,
    pin_d=CLEVIS_PIN_D,
    n_seg=32,
):
    ...
    for x_near, x_far in [
        ...
        angles = [2.0 * math.pi * i / n_seg for i in range(n_seg)]
        near_o = [
            bm.verts.new((x_near, ey + r_out * math.cos(a), ez + r_out * math.sin(a)))
            for a in angles
        ]
        far_o = [
            bm.verts.new((x_far, ey + r_out * math.cos(a), ez + r_out * math.sin(a)))
            for a in angles
        ]
        near_i = [
            bm.verts.new((x_near, ey + r_in * math.cos(a), ez + r_in * math.sin(a)))
            for a in angles
        ]
        far_i = [
            bm.verts.new((x_far, ey + r_in * math.cos(a), ez + r_in * math.sin(a)))
            for a in angles
        ]
        ...
        for i in range(n_seg):
            ...
            add_face(
                bm, [near_o[i], near_i[i], near_i[j], near_o[j]]
            )  # near cap annulus
...
def make_pinion_a_boss(
    name,
    bore_cx,
    bore_cy,
    z_center,
    y_offset=PINION_A_Y_OFFSET,
    boss_od=PINION_A_BOSS_OD,
    boss_id=PINION_A_BOSS_ID,
    half_len=None,
    n_seg=32,
):
    ...
    near_i = [
        bm.verts.new(
            (bore_cx - half_len, ey + r_in * math.cos(a), z_center + r_in * math.sin(a))
        )
        for a in angles
    ]
    far_i = [
...
def make_bevel_mount_bosses(
    ...
    boss_od=BEVEL_BOSS_OD,
    boss_h=BEVEL_BOSS_H,
    boss_bore=BEVEL_BOSS_BORE,
    n_seg=24,
):
    ...
    for x_off in [-spacing / 2.0, +spacing / 2.0]:
        ...
        no = [
            bm.verts.new(
                (x_c + r_o * math.cos(a), near_y, z_center + r_o * math.sin(a))
            )
            for a in angles
        ]
        fo = [
            bm.verts.new((x_c + r_o * math.cos(a), far_y, z_center + r_o * math.sin(a)))
            for a in angles
        ]
        ni = [
            bm.verts.new(
                (x_c + r_i * math.cos(a), near_y, z_center + r_i * math.sin(a))
            )
            for a in angles
        ]
        fi = [
            bm.verts.new((x_c + r_i * math.cos(a), far_y, z_center + r_i * math.sin(a)))
            for a in angles
        ]
        for i in range(n_seg):
            j = (i + 1) % n_seg
...
def make_shaft_conduit(
    ...
    bi = [
        bm.verts.new((cx_c + r_i * math.cos(a), cy_c + r_i * math.sin(a), z_from))
        for a in angles
    ]
    ti = [
        bm.verts.new((cx_c + r_i * math.cos(a), cy_c + r_i * math.sin(a), z_to))
        for a in angles
    ]
    bo = [
        bm.verts.new((cx_c + r_o * math.cos(a), cy_c + r_o * math.sin(a), z_from))
        for a in angles
    ]
    to = [
        bm.verts.new((cx_c + r_o * math.cos(a), cy_c + r_o * math.sin(a), z_to))
        for a in angles
    ]
...
def add_fin(
    bm, cx, cy, phi_center, r_hub, r_out, z_bot, h, t_half, vane_angle, swirl_dir
):
    """
...
for cfg in NACELLES:
    ...
    in_path = os.path.join(IN_DIR, str(cfg["in_stl"]))
    out_path = os.path.join(OUT_DIR, str(cfg["out_stl"]))
    ...
    shell_max_z = max(zs)
    print(
        f"  Shell Z range: {shell_min_z:.1f}..{shell_max_z:.1f} mm  "
    ...
    pivot_z = (
        ORIG_PIVOT_Z_FROM_INTAKE * axial_scale
    )  # Rev O: 83.0mm ref → ~103.75mm scaled
    ...
    bore_cx, bore_cy = compute_bore_center(
        nacelle, sample_z_lo, sample_z_hi, cfg["bore_cx"], cfg["bore_cy"]
    )
    print(
        f"  Computed bore centre: X={bore_cx:.2f} Y={bore_cy:.2f} "
        f"(sample Z {sample_z_lo:.1f}..{sample_z_hi:.1f})"
    )

    # ── Bore carving ──────────────────────────────────────────────────────────
    # BORE_CARVE_R=27.5mm = thrust tube OD (25mm bore + 2.5mm wall) — room for the sleeve.
    bore_removed = cut_bore_interior(
        nacelle, bore_cx, bore_cy, BORE_CARVE_R, 0.0, shell_max_z
    )
    print(
        f"  Carved interior bore: removed {bore_removed} faces  (Z=0..{shell_max_z:.0f}mm)"
    )

    # Widen carve at the exhaust end to strip the old fixed nozzle petal cone.
    petal_removed = cut_bore_interior(
        nacelle, bore_cx, bore_cy, 36.0, old_petal_start_z - 5.0, shell_max_z
    )
    print(
        f"  Stripped old fixed petals: removed {petal_removed} faces at R≤36mm, "
    ...
    removed_faces = cut_nozzle_shell_region(
        nacelle, bore_cx, bore_cy, ring_bottom_z, NOZZLE_CUT_R, cut_above=True
    )
    print(
        f"  Cleared nozzle ring pocket: removed {removed_faces} faces at Z>={ring_bottom_z:.1f}mm "
    ...
    print(
        f"  Thrust tube: Z={inlet_bell_z_throat:.0f}..{thrust_tube_z_top:.0f}mm  "
        f"ID={2 * R_FIN_OUT:.0f}mm  OD={2 * (R_FIN_OUT + THRUST_TUBE_WALL):.0f}mm"
    )

    # ── Bell-mouth inlet ──────────────────────────────────────────────────────
    inlet_bell = make_inlet_bell(
        "inlet_bell", bore_cx, bore_cy, 0.0, inlet_bell_z_throat
    )
    print(
        f"  Inlet bell: Z=0..{inlet_bell_z_throat:.0f}mm  "
    ...
    print(
        f"  Pivot boss: X-axis tilt hinge at Z={pivot_z:.1f}mm (Rev O CG-aligned), "
    ...
    print(
        f"  Pinion A boss: X-axis at Z={pivot_z:.1f}mm, Y_offset={PINION_A_Y_OFFSET:.1f}mm "
    ...
    print(
        f"  Bevel mount posts: Z={pivot_z:.1f}mm, spacing={BEVEL_BOSS_SPACING:.0f}mm X, "
    ...
    shaft_conduit = make_shaft_conduit(
        "shaft_conduit", bore_cx, bore_cy, pivot_z, crown_z
    )
    print(
        f"  Shaft conduit: Z={pivot_z:.1f}..{crown_z:.1f}mm, "
    ...
    crown_boss = make_pinion_a_boss(
        "crown_boss",
        bore_cx,
        bore_cy,
        crown_z,
        boss_od=CROWN_BOSS_OD,
        boss_id=CROWN_BOSS_ID,
        half_len=CROWN_BOSS_LEN / 2.0,
    )
    print(
        f"  Crown pinion boss: Z={crown_z:.1f}mm [MR63ZZ seats, same spec as Pinion A]"
    )

    # ── Motor mount struts ────────────────────────────────────────────────────
    # Xfly 2627: 26mm stator → ~30mm can → hub OD 32mm (R_HUB_OUT=16mm).
    aft_motor_mount = make_motor_mount(
        "aft_motor_mount",
        bore_cx,
        bore_cy,
        aft_mount_z,
    ...
    front_motor_mount = make_motor_mount(
        "front_motor_mount",
        bore_cx,
        bore_cy,
        front_mount_z,
    ...
    # ── Wire guides ───────────────────────────────────────────────────────────
    wire_guide_front_1 = make_wire_guide(
        "wire_guide_front_1",
        bore_cx,
        bore_cy,
        front_mount_z - 10.0,
    ...
    wire_guide_front_2 = make_wire_guide(
        "wire_guide_front_2",
        bore_cx,
        bore_cy,
        front_mount_z - 10.0,
    ...
    wire_guide_aft_1 = make_wire_guide(
        "wire_guide_aft_1",
        bore_cx,
        bore_cy,
        aft_mount_z - 10.0,
    ...
    wire_guide_aft_2 = make_wire_guide(
        "wire_guide_aft_2",
        bore_cx,
        bore_cy,
        aft_mount_z - 10.0,
    ...
    print(
        f"  Added nozzle ring at exhaust end (hinge Z={ring_bottom_z:.1f}mm): "
        f"{NOZZLE_RING_TEETH} teeth, {NOZZLE_RING_RACK_DEPTH:.1f}mm depth"
    )
    print(
        "  Nozzle iris is sized for the dual 50mm EDF path and is intended "
    ...
    combined.name = str(cfg["out_stl"]).replace(".stl", "")
    ...
    print(
        f"  Stator world: Z={stator_world_bot:.0f}..{stator_world_top:.0f}mm  "
    ...
    print(
        f"  EDF1 seat: Z={inlet_bell_z_throat:.0f}..{edf1_top:.0f}mm (intake end)  |  "
        f"EDF2 seat: Z={edf2_bot:.0f}..{thrust_tube_z_top:.0f}mm (exhaust end)"
    )
    print(
        f"  Nozzle ring hinge: Z={ring_bottom_z:.0f}mm  "
...
print(
    "Shells are 1.25× scale (50mm EDF); Z=0 = intake face, Z≈185.35mm = exhaust face."
)

print("\nRev O CHANGES vs Rev N:")
print(
    "  • Pivot Z moved from 74mm (ref) to 83mm (ref) — aligned to computed nacelle CG."
)
print(
    "    Benefit: zero first-moment of inertia about tilt axis; tilt servo only works against"
)
print(
    "    aerodynamic moment, not gravity moment.  Reduces servo stall current in hover."
)
print(
    "  • Pinion A bearing boss added at pivot Z: MR63ZZ press-fit (7mm OD, 3mm bore), X-axis."
)
print(
    "  • Bevel housing M2.5 mount posts (×2) added flanking Pinion A on inboard face."
)
print("  • Longitudinal shaft conduit rib added on nacelle exterior:")
print(
    f"    Route: pivot_Z → crown_Z={CROWN_PINION_Z_REF:.0f}mm (ref), "
...
print(
    f"  Full linkage ratio: (22/6) × (6/28) = {(22.0/6.0) * (6.0/28.0):.3f}  "
    f"→ 90° nacelle = {90.0 * (22.0/6.0) * (6.0/28.0):.1f}° ring travel."
)

print("\nEDF installation sequence:")
print(
    "  1. Press EDF1 in from INTAKE end (Z=0), seat at Z≈27.5mm, motor faces forward."
)
...
print(
    "  4. Press EDF2 in from EXHAUST end (Z≈185mm), seat at Z≈178.75mm, motor toward stator."
)
print("  5. Apply 3 dabs structural epoxy to EDF2 casing at Z≈122.5mm shoulder.")
print(
    "  6. Confirm stator fin vanes visible in gap Z≈90..123mm — stator is between EDFs."
)
print(
    "  7. Seat nozzle iris ring at ring_bottom_z; no separate hinge pins (rack-coupled)."
)

print("\nTILT HINGE (Rev O — CG-pivot):")
print(
    "  Pivot spar along X at (Y=0, Z=83mm) — bore-centre in Y = Y_cg; spar passes through inter-EDF gap bore."
)
print(
    "  MF104ZZ bearing bosses on both ±X faces of nacelle at (Y=0, Z=PIVOT_Z).  Pin = 4mm CF spar."
)
print(
    "  Pivot Y=0 (bore axis): eliminates gravity torque arm; tilt servo only works against aero/inertia loads."
)
print(
    "  Spar span = nacelle OD_X + 2×boss_depth = 60.5 + 10 = 70.5mm; fabricate arm bracket accordingly."
)
...
print(
    "  Spin-test each EDF before sealing nacelle; confirm rotation matches SWIRL_DIR."
)
