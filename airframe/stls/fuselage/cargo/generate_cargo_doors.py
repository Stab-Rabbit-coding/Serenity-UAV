...
def make_door(
    ...
    Returns
    -------
    (trimesh.Trimesh, float) — closed door solid, and z_hinge (the single
    straight-axis hinge Z this door's 4 knuckles share -- the CARGO-HINGE-
    SYNC single-source fact every other hinge-dependent generator/SCAD
    consumer must read from cargo_door_hinge_params.{py,scad}, not restate.
    """
...
# ---------------------------------------------------------------------------
# CARGO-HINGE-SYNC single-source output
# ---------------------------------------------------------------------------
# The hinge coordinates computed above (hinge_x_{port,stbd}, z_hinge_{port,
# stbd}) are read off the belly mesh EVERY run -- they shift whenever the
# cargo shell is re-merged (Rev T5-T5f did this repeatedly).  Before
# 2026-09-21 two other files hand-copied these numbers as their own
# constants (generate_cargo_hinge_retention.py's ROD_AXES,
# door_latch_mechanism.scad's HINGE_X_*/HINGE_Z_*) with a "keep in sync"
# comment instead of code -- both drifted stale after a shell re-merge and
# the shell-side retention bores stopped aligning with the door knuckle
# bores (found + fixed 2026-09-21, docs/CARGO_DOOR_LATCH_SPEC.md S7a).
#
# This function is the fix for the CLASS of bug, not just the instance: it
# writes the four numbers to ONE generated Python module (for Python
# consumers to import) and ONE generated OpenSCAD include (for SCAD
# consumers), matching the project's own cargo_layout_t5_params.scad
# pattern (tools/cargo_layout_fit.py --write-scad). Both are regenerated
# every time this script runs, so a stale copy can no longer exist as long
# as consumers import/include rather than restate.
HINGE_PARAMS_PY = os.path.join(SCRIPT_DIR, "cargo_door_hinge_params.py")
HINGE_PARAMS_SCAD = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR))),
    "openscad",
    "fuselage",
    "cargo",
    "cargo_door_hinge_params.scad",
)


def write_hinge_params(hx_port, hz_port, hx_stbd, hz_stbd) -> None:
    """Write the single-source hinge-coordinate outputs (Python + SCAD)."""
    py_text = (
        '"""cargo_door_hinge_params.py -- GENERATED by generate_cargo_doors.py.\n'
        "Do not edit: change the shell/generator and re-run "
        'generate_cargo_doors.py to regenerate.\n'
        "Hull frame, mm.  CARGO-HINGE-SYNC single source -- every other "
        "hinge-dependent\n"
        "generator or SCAD file reads this, not a hand-copied constant.\n"
        '"""\n\n'
        f"HINGE_X_PORT = {hx_port:.4f}\n"
        f"HINGE_Z_PORT = {hz_port:.4f}\n"
        f"HINGE_X_STBD = {hx_stbd:.4f}\n"
        f"HINGE_Z_STBD = {hz_stbd:.4f}\n"
        f"BAY_Y_FWD = {Y_BAY_FWD:.4f}\n"
        f"BAY_Y_AFT = {Y_BAY_AFT:.4f}\n"
        "ROD_AXES = {\n"
        f'    "port": (HINGE_X_PORT, HINGE_Z_PORT),\n'
        f'    "stbd": (HINGE_X_STBD, HINGE_Z_STBD),\n'
        "}\n"
    )
    with open(HINGE_PARAMS_PY, "w", encoding="utf-8") as fh:
        fh.write(py_text)
    ...
    scad_text = (
        ...
        f"HINGE_X_STBD = {hx_stbd:.4f};\n"
        f"HINGE_Z_STBD = {hz_stbd:.4f};\n"
        f"HINGE_BAY_Y_FWD = {Y_BAY_FWD:.4f};\n"
        f"HINGE_BAY_Y_AFT = {Y_BAY_AFT:.4f};\n"
    )
    with open(HINGE_PARAMS_SCAD, "w", encoding="utf-8") as fh:
        fh.write(scad_text)

    print(
        f"[hinge-params] wrote {HINGE_PARAMS_PY} and {HINGE_PARAMS_SCAD} "
        f"(port X={hx_port:.2f}/Z={hz_port:.2f}, stbd X={hx_stbd:.2f}/Z={hz_stbd:.2f})"
    )
...
def main() -> int:
    ...
    port_door, z_hinge_port = make_door(
        "port", belly_z, X_CL, hinge_x_port, KNUCKLE_Y_PORT
    )
    save(port_door, "cargo_door_port.stl")
    print()

    stbd_door, z_hinge_stbd = make_door(
        "stbd", belly_z, X_CL, hinge_x_stbd, KNUCKLE_Y_STBD
    )
    save(stbd_door, "cargo_door_stbd.stl")
    print()

    write_hinge_params(hinge_x_port, z_hinge_port, hinge_x_stbd, z_hinge_stbd)
    print()
