...
#     * PROFILE  = fwd_lap ∩ aft_lap — ⊆ BOTH sections' cavities at every station
#                  across the joint.  Extruded straight from y_lo to y_hi it gives
#                  ONE watertight ring that slips into both open mating faces with
#                  a bond gap everywhere.  collar∩shell is verified ≈ 0 by boolean
#                  below (the fit is correct by construction — no clipping).
...
#   (cargo/middle: cargo ≈ 1.3× the middle cross-section) the constant profile is
#   the NARROWER (middle) cavity, so the collar is snug in the middle and beds
#   into the wider cargo bore on a thicker West-System-406 fillet — the
#   documented asymmetric-fit pattern, acceptable because the joint is not
#   strength-limited (docs/structural_analysis.md §7.4).
#
...
def conforming_collar(fwd, aft, fwd_face, aft_face):
    """Build one continuous splice collar whose outer profile is the BRIDGE
    (fwd_lap ∩ aft_lap), constant over the full length.

    fwd_lap fits the fwd section over its 8 mm insertion span; aft_lap fits the
    aft section over its span; their intersection is therefore ⊆ BOTH sections'
    inner cavities at every station across the joint — so a constant prism of
    that profile slips into both sections with a bond gap everywhere, is a SINGLE
    connected body by construction, and never touches a wall (verified ≈ 0 by the
    collar∩shell boolean below — no shell subtraction, so it is immune to any
    shell mesh defect).  Where the two sections are similar (head/cargo,
    middle/rear) the profile hugs both walls (a tight 2-wall annulus on both
    sides); where they differ (cargo/middle: cargo ≈ 1.3× the middle cross-
    section) the collar is sized to the NARROWER section and beds into the wider
    one on a thicker West-System-406 fillet — the documented asymmetric-fit
    ...
    outer = _extrude(profile, y_lo, y_hi)
    inner = _extrude(hole, y_lo, y_hi)
