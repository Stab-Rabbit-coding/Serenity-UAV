...
the drone is controlled by 4 rp2350 modules with a custom sensor hat with GPS, imu, barometer, airspeed sensor connection, fpv camera, and control output for ESCs, nacelle and nozzle servos, payload release and winch, and navigation and anti-collision lights, all securedby a tpm.  jst gh connectors expose can fd (with selectable terminal jumper), mil-std-1553, rs485, and ethernet. 
these are networked to 4 raspberry cm4 with 4gb ram on a carrier board with a tpm, 2.4ghz and 5ghz wifi and zigbee, a tf slot for an os micro SD with a forensicly sound write blocker and a pi zero 2 x 20 gpio header, with internal and sma antenna connections.  all nodes are
...
✓This design document in all its revisions (A–H and beyond)
