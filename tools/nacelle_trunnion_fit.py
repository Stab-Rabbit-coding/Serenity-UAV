...
def main() -> int:
    ...
    sleeve_bore_r = (
        scad_const(pod, "SLEEVE_BORE_R_LITERAL") if False else scad_const(pod, "EDF_CASING_R") + 0.2
    )
    ...
    g.check(
        "T1a",
        pod_t_x0 >= DUCT_KEEPOUT_R,
        f"trunnion outboard face |X| = {pod_t_x0} >= {DUCT_KEEPOUT_R} " f"(Ø50 EDF bore + 1.0)",
    )
    pivot = scad_const(pod, "PIVOT_Z")
    in_sleeve = SLEEVE_Z_LO <= pivot <= SLEEVE_Z_HI
    g.check(
        "T1a2",
        (not in_sleeve) or pod_t_x0 >= SLEEVE_KEEPOUT_R,
    ...
    g.check(
        "T1b",
        gear_outboard >= DUCT_KEEPOUT_R,
        f"ring-gear outboard face |X| = {gear_outboard:.1f} " f">= {DUCT_KEEPOUT_R}",
    )
    g.check(
        "T1c",
        t_x0 == pod_t_x0,
        f"trunnion and pod agree on the spar-tip station " f"({t_x0} vs {pod_t_x0})",
    )
    g.check(
        "T1d",
        tip_face == pod_tip_face,
        f"trunnion and pod agree on the wing tip face " f"({tip_face} vs {pod_tip_face})",
    )
    ...
    g.check(
        "T2d",
        (t_x0 + flange_z + (magnet_face - (t_x0 + flange_z))) <= pilot_face,
    ...
    g.check(
        "T3a",
        abs(air_gap - w_air_gap) < 1e-6,
        f"trunnion AIR_GAP {air_gap} == wing HALL_AIR_GAP {w_air_gap}",
    )
    g.check(
        "T3b",
        abs(pad_proud - w_pad_proud) < 1e-6,
    ...
    g.check("T5b", abs(ratio - 3.571) < 5e-3, f"reduction = {ratio:.3f} (WA-R8: 3.571)")
    g.check(
        "T5c",
        abs(centre - 25.6) < 1e-6,
        f"centre distance = {centre:.2f} mm (WA-R8: 25.60 -> wing chord " f"station 53.6)",
    )
    ...
    g.check(
        "T6a",
        0.05 <= fit <= 0.25,
        f"register clearance = {fit:.2f} mm on diameter " f"(collar {pod_reg_d}, trunnion {reg_d})",
    )
    g.check("T6b", reg_d > brg_od, f"register Ø{reg_d} clears the Ø{brg_od} bearing seat inside it")

    # ---------------------------------------------------------- T7 magnet
    print("T7  ring magnet vs the built wing spec")
    g.check(
        "T7a",
        abs(mag_od - w_ring_od) < 1e-6,
        f"magnet OD {mag_od} == wing HALL_RING_OD {w_ring_od}",
    )
    g.check(
        "T7b",
        abs(mag_id - w_ring_id) < 1e-6,
        f"magnet ID {mag_id} == wing HALL_RING_ID {w_ring_id}",
    )
    mean_r = (mag_od + mag_id) / 4
    g.check(
        "T7c",
        abs(mean_r - w_sens_r) < 0.05,
        f"magnet mean radius {mean_r:.2f} == wing HALL_SENS_R {w_sens_r} — "
        f"the IC reads mid-annulus",
    )
    g.check(
        "T7d",
        scad_const(tru, "PILOT_OD") / 2 < w_pad_r,
    ...
    try:
        ...
        g.check("T8b", vol < 1e-6, f"solid overlap trunnion ∩ stator sleeve = {vol:.2f} mm³")
    except ImportError:
    ...
    for name, path in (("stator", stator), ("aft spider", aft)):
        ...
        g.check(
            "S1",
            abs((sleeve_bore_r * 2) - od - 0.4) < 1e-6,
            f"{name} sleeve OD {od} in a Ø{sleeve_bore_r*2:.1f} bore " f"= 0.20 mm/side clearance",
        )
        g.check(
            "S2a",
            scad_const(path, "SLEEVE_KEY_W") + 0.3 == slot_w,
            f"{name} key width {scad_const(path, 'SLEEVE_KEY_W')} vs slot " f"{slot_w}",
        )
        g.check(
            "S2b",
            scad_const(path, "SLEEVE_KEY_H") + 0.3 == slot_h,
            f"{name} key height {scad_const(path, 'SLEEVE_KEY_H')} vs slot " f"{slot_h}",
        )
        text = path.read_text(encoding="utf-8")
        ...
        g.check(
            "S3",
            has_call is None,
            f"{name} sleeve carries no spar bore or strut — the skewer is " f"gone from it",
        )
    ...
    try:
        ...
        for rel in (
            ...
            g.check(
                "M1",
                mesh.is_watertight and not neg,
    ...
    try:
        ...
        mesh.apply_translation([-scad_const(pod, "BORE_CX_L"), scad_const(pod, "BORE_CY"), 0.0])
        pivot = scad_const(pod, "PIVOT_Z")
        collar_x1 = t_x0 + 9.0
        ...
        g.note(
            "P1",
            f"trunnion collar stands at most {worst:.1f} mm proud of the "
