...
| AISI 4130 steel yield (rotating 8mm tilt-spar) | `docs/TILT_SPAR_ANALYSIS.md` §3/§7, `current-specification/bom_revS.csv` SPAR-TILT-4130 | Tilt-spar bending/torsion FOS use a typical 4130 normalized yield ~460 MPa (66.7 ksi); this allowable is not yet tied to a validated MMPDS product page. | Confirm the procured temper's design allowable against MMPDS-2023 (or the mill certificate) and add a `REF-MAT-*` catalog entry with a validated URL before spar procurement (TODO §0.x). |
