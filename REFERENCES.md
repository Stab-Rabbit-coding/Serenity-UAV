...
| **Stall current** | ⚠ Not verified for SPT5425LV. Budgeted at 1.2 A for the RAIL-2 sizing (spec §5.4); if actual stall exceeds ~2.5 A, RAIL-2 must be resized. **Note (2026-08-23):** the nacelle-tilt servos moved to DS3218, whose datasheet DOES publish stall current — **1.8 A @ 5 V, 2.2 A @ 6.8 V** — i.e. ~1.8× the 1.2 A placeholder. RAIL-2 sizing must be re-checked against 2.2 A per tilt servo. |
...
| DS3218 nacelle-tilt servo — torque shortfall vs the tilt requirement | `avionics/datasheets/DS3218 datasheet.pdf`, `docs/TILT_SPAR_ANALYSIS.md` §2, `current-specification/bom_revS.csv` SERVO-TILT | **Dimensions are now RESOLVED** — the datasheet was added 2026-08-23 and is authoritative (40 × 20 × 40.5 mm, 54.5 mm flange span, 49.5 × 10 mm bolt pattern, 60 g). What it also shows is a **torque gap**: DS3218 stall is **18 kgf·cm @ 5 V / 21.5 kgf·cm @ 6.8 V**, against the tilt-drive requirement of **≥ 25 kgf·cm (2.45 N·m)** (`serenity-rev-r.jsx` L383, cited in TILT_SPAR_ANALYSIS §2). At its own maximum rated voltage the part is ~14 % short, and more so at a 6 V rail. LibreServo_v4 re-drives the motor from its own 4.5–18 V input, so the converted servo may exceed the native figure — but **no torque number for the converted unit exists anywhere on file**. | Bench-measure stall torque of a DS3218 + LibreServo_v4 unit at the rail voltage actually chosen, and either show ≥ 25 kgf·cm or re-derive the tilt-drive requirement from the current nacelle CG and aero moments. Do not assume the conversion closes the gap. (root `TODO.md` §1.1.3) |
