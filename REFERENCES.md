...
- [Part XII — Sensor and Component Specifications](#part-xii--sensor-and-component-specifications)
    ...
    - [REF-ACT-003: TowerPro SG90 — 9 g Micro Servo (cargo door / release; RCS valves Phase 11)](#ref-act-003-towerpro-sg90--9-g-micro-servo-cargo-door--release-rcs-valves-phase-11)
    - [REF-ESC-001: Open-Secure-ESC — 6S/10A brushed build, the nacelle-tilt controller](#ref-esc-001-open-secure-esc--6s10a-brushed-build-the-nacelle-tilt-controller)
...
**Used in:** `docs/structural_analysis.md` §7.3, `airframe/fuselage-mid/WBS.md` §1.1.1.2 CARGO-03c, `docs/CARGO_DOOR_LATCH_SPEC.md` §3 (door-latch hook/lip bearing and shear, same VERIFY-bearing-proxy treatment), this file's Open Standards Verification Items table (CF-PETG bearing allowable row)
...
**Re-read 2026-09-21** (repository README, for `docs/CARGO_DOOR_GATEWAY_SPEC.md`): status
unchanged — "In active development. Nothing here is shippable yet."; the OSC Dev CH32
board is validated at Rev B and the protocol/bus transport "bench-proven on real silicon";
the **servo swap board is "designed but not spun yet"**; control loops pending. The README
does **not** state the osc-native physical layer (single-wire half-duplex TTL with a
direction line, or full-duplex UART) or the swap board's logic level — both are
verification items for the gateway's `J_FLEX` wiring (see "Open Standards Verification
Items").

**Applied to:** `SERVO-CARGO` (port door ×1, starboard door ×1, payload release ×1 — qty 3
since 2026-09-21, `docs/CARGO_DOOR_GATEWAY_SPEC.md` D-GW-2), hosted by `CAN-PERIPH-GW-DOOR`;
RCS bleed-jet proportional valve servos (`SERVO-RCS-VALVE`, ×4, Phase 11 deferred) are the
same SG90 class and inherit this note when that phase is implemented (hosted by the Phase 11
`GW-RCS` gateway specified in that document §9), but are not otherwise touched here.
...
### REF-ACT-003: TowerPro SG90 — 9 g Micro Servo (cargo door / release; RCS valves Phase 11)

| Field | Value |
|---|---|
| **Manufacturer** | TowerPro (Tower Pro Pte Ltd), Taiwan |
| **Product page** | <https://www.towerpro.com.tw/product/sg90-7/> |
| **Retrieved** | 2026-09-21 (product page read directly; TowerPro publishes no separate datasheet PDF for this part) |
| **Figures used** | Dimensions "23×12.2x29mm"; weight "9g"; stall torque "1.8kg/cm(4.8v)"; operating speed "0.1sec/60degree(4.8v)"; operating voltage "4.8v". Operating current is stated only in the manufacturer's Q&A on that page: "about 0.5A – 2A" — carried as the range to size the servo-branch fuse against, **not** as a datasheet stall figure. No PWM input-threshold or 6 V rating is published. |
| **Applied to** | `docs/CARGO_DOOR_GATEWAY_SPEC.md` (D-GW-4 servo-power branch, §3 logic-level VERIFY, §9 RCS valve dynamics), `docs/CARGO_DOOR_LATCH_SPEC.md` §1/§2 (stall-torque-vs-V_max governing finding, pushrod force), `current-specification/bom_revS.csv` `SERVO-CARGO` / `SERVO-RCS-VALVE`, `docs/POWER_DISTRIBUTION.md` §3.3 |
...
| Field | Value |
|---|---|
| **Project** | Open-Secure-ESC — *Open Source Trustworthy Electronic Speed Control* (Griffing Technology LLC; same author as this repository) |
| **Repository** | <https://github.com/Stab-Rabbit-coding/Open-Secure-ESC> — local checkout `../Open-Secure-ESC` |
| **Build instance** | `builds/6s/10A/BRUSHED_CAN_485_isolation/` — 6S, 10 A tier, Brushed (DC), magnetic-absolute remote shaft sensor, Holding Brake (low-side solenoid driver), CAN-FD + RS-485, Isolation EMI tier; branch `feat/tilt-controller-6s-10a-brushed` (2026-09-17) until merged |
| **License** | CERN Open Hardware Licence Version 2 — Permissive (CERN-OHL-P v2) — <https://ohwr.org/cern_ohl_p_v2.txt> |
| **What it is** | The per-side nacelle-tilt controller: TI DRV8874-Q1 integrated H-bridge (that repository's `REFERENCES.md` [62] SLVSF66A / [63] SLVSF67B), TPS54560B buck motor/solenoid rail and 3V3 ([65] SLVSF00), TPL7407L brake-solenoid low-side driver ([66] SLRS066D), remote Broadcom AEAT-8800-Q24 worm-shaft encoder header ([64] pub-005892), ADM3057E/ADM2587E isolated CAN-FD/RS-485, MSPM0G3518-Q1 RHB ([44]) + OPTIGA™ Trust M ([45]). Every part cited there from a locally held primary datasheet; this entry inherits that chain rather than repeating it |
| **Status (2026-09-17)** | Schematic ERC 0 errors / 0 warnings, netlist verified, BOM walked from the decision matrix; **no PCB yet** (that repository's `TODO.md` §18: inductor/diode selection under the 4 mm height budget, buck values, ITRIP, origin verification, layout, firmware) |
| **Host constraints carried by the build** | ≤ 42.9 × 36.5 mm outline, ≤ 4 mm parts on the web-facing side, bare back, connectors aft/inboard (`airframe/openscad/fuselage/cargo/tilt_actuator_bracket.scad` Rev T5e); VBAT branch F_TILT_P/S 3 A; Pololu 20D #3712 (REF-ACT-001); solenoid BRK-4; starboard sense reversed |
| **Fail-state and firmware contract** | The build README's fail-state table and "Firmware requirements" own the controller-side behaviour that `docs/TILT_DRIVE_CONTROL_SPEC.md` §5.2–§5.5 cites (nFAULT → brake engage, rail-droop rule, latched OCP, brake-release command class with asymmetric MAC fail policy) |
| **Supersedes** | The LibreServo_v4.1-TC variant proposal (`LibreServo_v4/docs/CR-2026-09-15-tilt-controller-variant.md`, now marked SUPERSEDED) — see "Removed / Superseded Citations" |
...
| OpenServoCore osc-native physical layer and logic level (REF-SENSOR-015) | `docs/CARGO_DOOR_GATEWAY_SPEC.md` §3 (GW-DOOR-2), `avionics/kicad/CAN-PERIPH-GW-1/CAN-PERIPH-GW-1.md` Deployment 4 | The upstream README (re-read 2026-09-21) does not state whether osc-native is single-wire half-duplex TTL (needing a direction/enable line — `FLEX_TTL_GPIO` is reserved for it) or full-duplex, nor the swap board's I/O voltage. The gateway MCU is 3.3 V. | Read the upstream hardware docs / schematic for the bus physical layer and I/O level before wiring `J_FLEX`; decide the `FLEX_TTL_GPIO` role; re-check the shippable-release gate literally. |
| Door-latch hook/lip bearing + shear allowable (REF-MAT-001 used as a proxy) | `docs/CARGO_DOOR_LATCH_SPEC.md` §3 (DOOR-LATCH-1) | REF-MAT-001 is a bulk tensile/compressive coupon test, not a bearing- or shear-specific test; FDM layers are anisotropic so the distortion-energy shear estimate is order-of-magnitude only. | Bench pull-test the hook to the 60.5 N / 0.605 N.m design load before flight-article procurement; source a bearing-specific coupon test if available. |
| SG90 PWM input at 3.3 V; SG90 on the 6 V servo rail (REF-ACT-003) | `docs/CARGO_DOOR_GATEWAY_SPEC.md` §3, §8 (GW-DOOR-3, GW-DOOR-5), `docs/POWER_DISTRIBUTION.md` §3.3 | TowerPro publishes no PWM input threshold and lists 4.8 V only; the PWM fallback drives the servo from a 3.3 V MCU pin, and §3.3 already places the SG90 class on the 6 V rail. | Bench: confirm reliable command at 3.3 V (else a harness level shifter); confirm the sourced servo's 6 V rating or feed `F_DOOR` from 5 V; measure the three-servo stall current to size `F_DOOR`. |
| MSPM0G3518-Q1 PA24 timer function (REF-SENSOR-017) | `docs/CARGO_DOOR_GATEWAY_SPEC.md` §3 (`FLEX_TTL_GPIO` = PA24 as the release-servo PWM in the fallback) | Whether PA24 offers a timer-channel pinmux on the RHB-32 was not read from SLASFA6B Table 6-2; only PA25 (TIMA0_C3) and PA26 (TIMG8_C0) were verified at the 2026-08-03 retarget. | Read Table 6-2 for PA24; if no timer function, drive the release servo with a software-timed 50 Hz pulse (acceptable for a one-shot command) or put it on the osc-native chain. |
