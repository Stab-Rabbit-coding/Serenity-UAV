...
| Wing/nacelle Hall tilt encoder — part not yet selected | `current-specification/bom_revS.csv` HALL-TILT-ENC, `docs/TILT_SPAR_ANALYSIS.md` §1/§3.5, `avionics/WBS.md` §1.9.1 | The tilt-feedback encoder is specified by function only (**off-axis-capable** magnetic angle IC, candidate MT6701 / MPS MA732). No specific datasheet, I²C-address/pinout, or off-axis air-gap spec is vetted; the geometry (`HALL_*` in `wings_s1223_revo.scad` / `_export_pivot_slab.scad`) is provisional. | Select a real off-axis encoder, confirm its datasheet pinout/protocol, off-axis magnet spec, and behaviour with a ferrous through-shaft, and add a `REF-SENSOR-*` entry with a validated URL before PCB/harness sign-off (TODO §0.8). Do not procure against the placeholder. |
