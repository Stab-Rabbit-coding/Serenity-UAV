...
// ── Rev M weight budget (PocketBeagle 2 Industrial upgrade — AM6254, 1GB, 64GB eMMC) ──
const W_ITEMS = [
  // Hull structure
  ...
  ["120A ESC ×4 (nacelle, one per EDF, HW Platinum PRO V4 120A)", 428],
  ...
  // Avionics — 8× PocketBeagle 2 Industrial + Cape-A/B (Rev M hardware upgrade)
  ["PocketBeagle 2 Industrial ×8 (AM6254 + PRU-ICSS)", 104],
  ["Cape-A Sensor/Flight PCB ×4 (85×55mm 4L)", 112],
  ["Cape-B Comms/Payload PCB ×4 (90×60mm 4L)", 152],
  ["RCRS-49 sub-modules ×4",                    48],
  ["microSD ×4 (log only — eMMC boot on PB2-I)", 4],
  // Power + wiring
  ["PDB + BEC 5V/5A",                          30],
...
function HullProfileDiagram(){
  ...
  return(
    <svg viewBox={`0 0 ${VW} ${VH}`} width="100%" style={{maxWidth:"100%",display:"block"}}>
      ...
      <text x={VW-119} y={24} textAnchor="middle" fill={C.lime} fontSize={8} fontFamily={M} letterSpacing="0.5">REV M — DUAL 80mm 6S SERIES</text>
      <text x={VW-119} y={38} textAnchor="middle" fill={C.lime} fontSize={9} fontFamily={M} fontWeight="bold">PB2 INDUSTRIAL · PID GOVERNOR</text>
      ...
      <text x={VW/2} y={14} textAnchor="middle"
        ...
        TOP VIEW — CANONICAL PROPORTIONS — REV M (DUAL-EDF + PID GOVERNOR + PB2 INDUSTRIAL)</text>
    </svg>
  );
}
  ...
  return(<div>
    <div style={{background:"rgba(163,230,53,0.07)",border:"1px solid rgba(163,230,53,0.4)",
      ...
      <div style={{color:C.lime,fontFamily:M,fontSize:12,fontWeight:"bold",marginBottom:6,letterSpacing:"0.08em"}}>
        REV M — 18" CANONICAL · DUAL 80mm 6S SERIES EDFs PER NACELLE · PID GOVERNOR · 8× PocketBeagle 2 Industrial
      </div>
      <div style={{color:C.gold,fontFamily:M,fontSize:10,marginBottom:12}}>
        Supersedes Rev L · Hardware upgrade: PocketBeagle 2 → PocketBeagle 2 Industrial (AM6254, 1GB DDR4, 64GB eMMC, −40°C to 85°C) · Propulsion + governor unchanged
      </div>
    ...
    {/* Rev L → Rev M delta */}
    <SH t="Rev L → Rev M Changes (Hardware Upgrade)"/>
    <div style={{overflowX:"auto",marginBottom:16}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:M,fontSize:10}}>
        <TH cols={["Change","Rev L","Rev M"]}/>
        <tbody>
          {[
            ["Compute board","PocketBeagle 2 (AM6232)","PocketBeagle 2 Industrial (AM6254)"],
            ["SoC cores","Dual Cortex-A53 (1.0GHz)","Quad Cortex-A53 (1.4GHz) — 2× headroom"],
            ["RAM","512MB DDR4","1GB DDR4"],
            ["eMMC","Not populated — OS from microSD","64GB eMMC — OS boot + storage on-board"],
            ["OS microSD (8× nodes)","Required — 1× per node for OS boot","Eliminated — eMMC handles OS boot"],
            ["Log microSD (4× Cape-B)","Write-blocked flight log (unchanged)","Write-blocked flight log (unchanged)"],
            ["Operating temperature","Commercial 0°C to 70°C","Industrial −40°C to 85°C"],
            ["Onboard MCU","None beyond A53/M4F/PRU","MSPM0L1105 + 12-bit ADC (future expansion)"],
            ["LiPo charger","External only","Onboard LiPo charger (future use)"],
            ["Cape-A / Cape-B headers","72-pin expansion — Rev K design","Unchanged — same 72-pin expansion, full compatibility"],
            ["Propulsion (EDFs/ESCs)","Dual 80mm 6S per nacelle · 4× 120A","Unchanged"],
            ["PID governor firmware","Rev L — M4F 500Hz","Unchanged — binary compatible"],
            ...
            <tr key={c}>
              ...
              <td style={{padding:"4px 9px",borderBottom:`1px solid ${C.border}`,color:C.dimmer,opacity:0.7}}>{l}</td>
              <td style={{padding:"4px 9px",borderBottom:`1px solid ${C.border}`,color:C.green}}>{m}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    {/* Rev K → Rev L historical */}
    <SH t="Rev K → Rev L Changes (Historical)"/>
    <div style={{overflowX:"auto",marginBottom:16}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:M,fontSize:10}}>
        ...
        <tbody>
          {[
            ...
            <tr key={c}>
              ...
              <td style={{padding:"4px 9px",borderBottom:`1px solid ${C.border}`,color:C.teal}}>{l}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      ...
      <div>
        <KV k="MCU" v="PocketBeagle 2 Industrial AM6254" vc={C.text}/>
        <KV k="SoC cores" v="4× Cortex-A53 1.4GHz + M4F + PRU-ICSS" vc={C.text}/>
        <KV k="RAM / eMMC" v="1GB DDR4 · 64GB eMMC (no OS microSD)" vc={C.text}/>
        ...
        <KV k="Temp range" v="−40°C to 85°C industrial" vc={C.text}/>
        ...
        <KV k="Avionics nodes" v="8× PocketBeagle 2 Industrial — Cape-A/B" vc={C.text}/>
      </div>
    </div>
    ...
    <Note ch="Rev M hardware upgrade: 8× PocketBeagle 2 (AM6232) replaced by 8× PocketBeagle 2 Industrial (AM6254). Cape-A and Cape-B PCBs unchanged — same 72-pin expansion headers. 8× OS microSD cards eliminated (eMMC handles OS boot). 4× Cape-B write-blocked log microSD cards retained. Governor firmware binary unchanged from Rev L — fully compatible with AM6254 M4F."/>
    <Good ch="PB2 Industrial in stock at DigiKey (2820-100003007-ND, $51.03 ea) — no long lead time for standard quantities."/>
    <Good ch="Industrial −40°C to 85°C range covers all expected UAV operating environments including cold-weather ops."/>
  </div>);
}
...
function AvionicsTab(){
  return(<div>
    <SH t="8-Node PocketBeagle 2 Industrial Architecture (Rev M — AM6254)"/>
    <div style={{background:"rgba(163,230,53,0.07)",border:"1px solid rgba(163,230,53,0.4)",
      borderRadius:6,padding:"12px 16px",marginBottom:16}}>
      <div style={{color:C.lime,fontFamily:M,fontSize:11,fontWeight:"bold",marginBottom:4}}>
        REV M HARDWARE CHANGE — PocketBeagle 2 → PocketBeagle 2 Industrial
      </div>
      <div style={{color:C.dim,fontFamily:M,fontSize:10,lineHeight:1.9}}>
        All 8 nodes upgraded to AM6254 (quad Cortex-A53 1.4GHz + M4F + PRU-ICSS). Cape-A and Cape-B PCBs unchanged — same 72-pin expansion headers.
        8× OS microSD cards eliminated; each board boots from its own 64GB eMMC. 4× Cape-B write-blocked log microSD cards retained.
        Rev L governor firmware (governor_firmware_revL.bin) runs unchanged on the AM6254 M4F coprocessor — binary compatible.
      </div>
    </div>

    <SH t="PocketBeagle 2 Industrial — Board Specs"/>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:4}}>
      <div>
        <KV k="Part number (MFR)" v="100003007" vc={C.teal}/>
        <KV k="Part number (DigiKey)" v="2820-100003007-ND" vc={C.teal}/>
        <KV k="Unit price" v="$51.03 · In stock (DigiKey)" vc={C.green}/>
        <KV k="8× boards cost" v="$408.24 (vs $316.24 standard PB2)" vc={C.yellow}/>
        <KV k="SoC" v="TI AM6254 (4× Cortex-A53 up to 1.4GHz)" vc={C.orange}/>
        <KV k="Coprocessors" v="Cortex-M4F + PRU-ICSS (250MHz)" vc={C.orange}/>
        <KV k="RAM" v="1GB DDR4" vc={C.green}/>
        <KV k="eMMC" v="64GB (OS boot + storage)" vc={C.green}/>
      </div>
      <div>
        <KV k="Operating temp" v="−40°C to 85°C industrial" vc={C.lime}/>
        <KV k="Form factor" v="55 × 35 mm · 12.7g" vc={C.text}/>
        <KV k="Expansion" v="72-pin headers (Cape-A/B compatible)" vc={C.text}/>
        <KV k="Onboard MCU" v="MSPM0L1105 + 12-bit ADC (future use)" vc={C.purple}/>
        <KV k="LiPo charger" v="Onboard (future use)" vc={C.purple}/>
        <KV k="USB" v="USB-C power + 3.3V JST-SH UART debug" vc={C.text}/>
        <KV k="OS microSD" v="Not required — eMMC boots OS" vc={C.green}/>
        <KV k="Headers" v="Pre-soldered (production reliability)" vc={C.text}/>
      </div>
    </div>

    <SH t="8-Node Architecture"/>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <div>
        ...
        <KV k="M4F governor (Rev L firmware)" v="500Hz PID per EDF — all FC nodes" vc={C.green}/>
    ...
    <SH t="Rev M Board Swap + Firmware Procedure"/>
    <ol style={{fontFamily:M,fontSize:10,lineHeight:2.1,color:C.dim,paddingLeft:20,marginTop:8}}>
      <li>Power down. Remove all 8 PocketBeagle 2 (AM6232) boards from Cape-A/B stacks.</li>
      <li>Seat 8× PocketBeagle 2 Industrial (AM6254, 100003007) onto same Cape-A/B 72-pin headers.</li>
      <li>Flash OS image to eMMC on each board via USB-C + USB drive (no microSD needed).</li>
      <li>Remove all 8 OS microSD cards — not needed with eMMC. Retain 4× Cape-B log microSD cards.</li>
      <li>Upload <span style={{color:C.teal}}>governor_firmware_revL.bin</span> via MAVLink to CN1 (binary unchanged from Rev L).</li>
      ...
      <li>Reboot all 4 FC nodes. Confirm: <span style={{color:C.teal}}>GOV_STATE = NORMAL</span> on all 4 ESCs via MAVLink.</li>
    </ol>

    <Good ch="TPM 2.0 signature check prevents rogue firmware installation. CPLD write-blocker protects flight logs."/>
    <Good ch="AM6254 is binary-compatible with AM6232 for the M4F governor partition. No firmware recompile needed."/>
    <Good ch="64GB eMMC eliminates 8 OS microSD failure points. Industrial temp range eliminates thermal throttling concern at high ambient temps."/>
    <Warn ch="M4F governor update and A53 OS update are independent partitions — do not flash OS update as M4F firmware."/>
    <Note ch="MSPM0L1105 onboard MCU with 12-bit ADC is available for future analog sensor integration. Not used in current Cape-A/B designs."/>
  </div>);
}
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <div>
        ...
        <KV k="Total thrust" v={`${THRUST}g (${gToLb(THRUST)} lb)`} vc={C.green}/>
        <KV k="Nacelle ESC" v="Hobbywing Platinum PRO V4 120A ×4"/>
      ...
      <div>
        ...
        <KV k="T/W one nacelle lost" v="1.62:1 — controlled descent" vc={C.yellow}/>

        <SH t="Governor (Rev L — unchanged in Rev M)" c={C.green} mt={16}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      ...
      <div>
        ...
        <KV k="Nav light driver" v="PCA9685 I²C PWM (0x40) — unchanged"/>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:M,fontSize:10}}>
        ...
        <tbody>
          {[
            ...
            ["M ★","PocketBeagle 2 Industrial (AM6254) · 1GB DDR4 · 64GB eMMC · −40°C industrial","11,250g",TW_EMPTY],
            ...
            <tr key={rev}>
              ...
              <td style={{padding:"5px 9px",borderBottom:`1px solid ${C.border}`,color:parseFloat(tw)>=2.5?C.green:C.yellow}}>{typeof tw==="number"?tw.toFixed(2):tw}:1</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{marginBottom:18}}>
          ...
          <div style={{color:C.orange,fontFamily:M,fontSize:13,marginTop:4}}>
            Rev M — 18" Canonical · Dual 80mm 6S Series EDFs per Nacelle · PID Governor · 8× PocketBeagle 2 Industrial (AM6254)
          </div>
          ...
          <div style={{display:"flex",gap:20,marginTop:8,flexWrap:"wrap"}}>
            ...
            <span style={{fontFamily:M,fontSize:10,color:C.lime}}>AM6254 quad A53 · 1GB · 64GB eMMC</span>
          </div>
        </div>
        ...
        <div style={{marginTop:32,paddingTop:12,borderTop:`1px solid ${C.border}`,
          ...
          Serenity-Class Tiltrotor UAV · Rev M · CC BY 4.0 · Steve Griffing 2026
        </div>
      </div>
    </div>
  );
