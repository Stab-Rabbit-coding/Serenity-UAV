...
def add_silk_label(board, text, x_mm, y_mm, size=0.65, thickness=0.10,
                   angle_deg=0, layer=pcbnew.F_SilkS, mirror=False):
...
def main():
    ...
    # -----------------------------------------------------------------------
    for n in ("DDS_RF", "PA_INT", "RF_LPF_N1", "RF_LPF_N2",
              "RF_FILT", "RX_LNA", "TCXO_OUT",
              "RSSI_RAW", "RF_ANT_SW", "COMP_IN"):
    ...
    # -----------------------------------------------------------------------
    u1 = load_fp("Package_SO", "MSOP-10_3x3mm_P0.5mm",
                 "U1", "Si5351A-B-GT  49MHz DDS I2C Synthesiser",
                 115.0, 112.0)
    assign_pads(board, u1, {
        "1":  "+3V3",     "2":  "GND",
        "3":  "DDS_RF",   "4":  "GND",       "5":  "GND",
        "6":  "DDS_CLK",  "7":  "DDS_DAT",
        "8":  "TCXO_OUT", "9":  "GND",       "10": "+3V3",
    })
    board.Add(u1)

    # X1 — EPSON TG2520SMN 25 MHz ±0.5 ppm TCXO  (4-pad SMD 2520)
    # Pinout: 1=/OE(tie VDD) 2=GND 3=CLK_OUT 4=VDD
    x1 = load_fp("Oscillator",
                 "Oscillator_SMD_SeikoEpson_TG2520SMN-xxx-xxxxxx-4Pin_2.5x2.0mm",
                 "X1", "TG2520SMN  25MHz TCXO ±0.5ppm",
                 115.0, 106.5)
    assign_pads(board, x1, {
        "1": "+3V3", "2": "GND", "3": "TCXO_OUT", "4": "+3V3",
    })
    board.Add(x1)

    # U2A — MCP4921 12-bit SPI DAC  (SOT-23-8) — Bell 202 TX audio
    # Pinout: 1=VDD 2=/CS 3=SCK 4=SDI 5=/LDAC 6=VREF 7=VOUTA 8=GND
    u2a = load_fp("Package_TO_SOT_SMD", "SOT-23-8",
                  "U2A", "MCP4921  12-bit SPI DAC  TX-AFSK Audio",
                  122.0, 112.0)
    assign_pads(board, u2a, {
        "1": "+3V3",     "2": "DDS_FSYNC",
        "3": "DDS_CLK",  "4": "DDS_DAT",
        "5": "GND",      "6": "+3V3",
        "7": "AFSK_OUT", "8": "GND",
    ...
    u2b = load_fp("Package_TO_SOT_SMD", "SOT-23-5",
                  "U2B", "LM393  Comparator  RX-AFSK Demodulator",
                  122.0, 119.0)
    assign_pads(board, u2b, {
        "1": "AFSK_IN",  "2": "+3V3",
        "3": "COMP_IN",  "4": "GND",   "5": "GND",
    })
    board.Add(u2b)

    # U3A — MMBT2222A NPN BJT PA driver  (SOT-23)
    # Pinout: 1=B(base) 2=E(emitter) 3=C(collector)
    u3a = load_fp("Package_TO_SOT_SMD", "SOT-23",
                  "U3A", "MMBT2222A  NPN BJT  PA Class-A Driver",
                  133.0, 112.0)
    assign_pads(board, u3a, {
        "1": "DDS_RF",  "2": "GND",  "3": "PA_INT",
    })
    board.Add(u3a)

    # U3B — 2N3866 NPN BJT PA final  (SOT-89-3, collector tab)
    # Pinout: 1=B 2=E 3=C(tab) — tab bonded to GND pour for heatsinking
    u3b = load_fp("Package_TO_SOT_SMD", "SOT-89-3",
                  "U3B", "2N3866  NPN BJT  PA Class-AB Final 100mW",
                  137.5, 112.0)
    assign_pads(board, u3b, {
        "1": "PA_INT",  "2": "GND",  "3": "RF_TX",
    })
    board.Add(u3b)
    ...
    # -----------------------------------------------------------------------
    fl1_l1 = load_fp("Inductor_SMD", "L_0805_2012Metric",
                     "FL1_L1", "100nH  Coilcraft-0805HQ-101J  LPF Series-1",
                     132.0, 117.0)
    assign_pads(board, fl1_l1, {"1": "RF_TX",      "2": "RF_LPF_N1"})
    board.Add(fl1_l1)

    fl1_c1 = load_fp("Capacitor_SMD", "C_0402_1005Metric",
                     "FL1_C1", "120pF C0G  LPF Shunt-1",
                     132.0, 120.0)
    assign_pads(board, fl1_c1, {"1": "RF_LPF_N1",  "2": "GND"})
    board.Add(fl1_c1)

    fl1_l2 = load_fp("Inductor_SMD", "L_0805_2012Metric",
                     "FL1_L2", "180nH  Coilcraft-0805HQ-181J  LPF Series-2",
                     136.0, 117.0, rot_deg=90)
    assign_pads(board, fl1_l2, {"1": "RF_LPF_N1",  "2": "RF_LPF_N2"})
    board.Add(fl1_l2)

    fl1_c2 = load_fp("Capacitor_SMD", "C_0402_1005Metric",
                     "FL1_C2", "180pF C0G  LPF Shunt-2",
                     136.0, 120.0)
    assign_pads(board, fl1_c2, {"1": "RF_LPF_N2",  "2": "GND"})
    board.Add(fl1_c2)

    fl1_l3 = load_fp("Inductor_SMD", "L_0805_2012Metric",
                     "FL1_L3", "180nH  Coilcraft-0805HQ-181J  LPF Series-3",
                     140.0, 117.0)
    assign_pads(board, fl1_l3, {"1": "RF_LPF_N2",  "2": "RF_FILT"})
    board.Add(fl1_l3)

    fl1_c3 = load_fp("Capacitor_SMD", "C_0402_1005Metric",
                     "FL1_C3", "120pF C0G  LPF Shunt-3",
                     140.0, 120.0)
    ...
    # -----------------------------------------------------------------------
    u5 = load_fp("Package_TO_SOT_SMD", "SOT-363_SC-70-6",
                 "U5", "PE4259-63  SPDT TX/RX Switch  ≥35dB isolation",
                 143.5, 117.5)
    assign_pads(board, u5, {
        "1": "RF_ANT_SW",  "2": "GND",
        "3": "RF_RX",      "4": "PTT_N",
        "5": "RF_FILT",    "6": "GND",
    })
    ...
    # -----------------------------------------------------------------------
    u4 = load_fp("Package_TO_SOT_SMD", "SOT-363_SC-70-6",
                 "U4", "MGA-82563  LNA  2.4dB NF  50-6000MHz",
                 143.5, 123.5)
    assign_pads(board, u4, {
        "1": "RX_LNA",   "2": "RSSI_RAW",
        "3": "RF_RX",    "4": "+3V3",
        "5": "GND",      "6": "GND",
    })
    ...
    # -----------------------------------------------------------------------
    u6 = load_fp("Package_TO_SOT_SMD", "SOT-23-5",
                 "U6", "MCP1703T-3302  LDO 3.3V  72dB-PSRR@100kHz",
                 117.0, 122.5)
    assign_pads(board, u6, {
        "1": "+5V_FILT",  "2": "GND",
        "3": "+3V3",      "4": "+3V3",   "5": "GND",
    })
    board.Add(u6)
    ...
    tvs6 = load_fp("Package_TO_SOT_SMD", "SOT-363_SC-70-6",
                   "TVS6", "PRTR5V0U2X  Dual TVS 5.5V  UART-TX/RX ESD",
                   104.5, 110.5)
    assign_pads(board, tvs6, {
        "1": "GND",       "2": "UART_TX",
        "3": "GND",       "4": "GND",
        "5": "UART_RX",   "6": "GND",
    })
    ...
    cm5 = load_fp("Inductor_SMD", "L_0402_1005Metric",
                  "CM5", "SRF2012-100Y  CMC  UART TX/RX Pair 10uH",
                  108.5, 110.5)
    ...
    for ref, y_pos, net in [
        ...
        fbtx = load_fp("Inductor_SMD", "L_0402_1005Metric",
                       ref, "742792510  0402 Ferrite Bead 600Ohm@100MHz",
                       113.0, y_pos)
    ...
    tvs_sma = load_fp("Diode_SMD", "D_SOD-123F",
                      "TVS_SMA", "SMAJ5.0A  Bidirectional TVS 5V  Antenna ESD",
                      151.5, 116.5)
    ...
    # -----------------------------------------------------------------------
    supply_caps = [
        ("C21", "100uF 10V X5R 1210  +5V Bulk",         "C_1210_3225Metric",  110.5, 121.0, "+5V_FILT", "GND"),
        ("C22", "100nF X7R 0402  +5V HF Bypass",         "C_0402_1005Metric",  113.0, 121.0, "+5V_FILT", "GND"),
        ("C23", "100uF 10V X5R 1210  +3V3 Bulk",         "C_1210_3225Metric",  121.0, 122.5, "+3V3",     "GND"),
        ("C24", "10uF X5R 0805  +3V3 Mid-freq",          "C_0805_2012Metric",  124.5, 122.5, "+3V3",     "GND"),
        ("C25", "100nF X7R 0402  +3V3 HF Bypass",        "C_0402_1005Metric",  127.0, 122.5, "+3V3",     "GND"),
        ("C26", "10nF C0G 0402  +3V3 VHF 49MHz Bypass",  "C_0402_1005Metric",  128.5, 122.5, "+3V3",     "GND"),
        ("C27", "10nF C0G 0402  GND-PGND Moat Bridge",   "C_0402_1005Metric",  130.0, 117.0, "GND",      "GND"),
    ]
    ...
    if u1_clk0_x and u3a_base_x:
        add_track(board, FCU, "DDS_RF",
                  u1_clk0_x, u1_clk0_y, u3a_base_x, u3a_base_y, SIG_W)
    ...
    if u3b_col_x and l1_in_x:
        ...
        add_track(board, FCU, "RF_TX",
                  u3b_col_x, u3b_col_y, mid_x, u3b_col_y, RF_W)
        add_track(board, FCU, "RF_TX",
                  mid_x, u3b_col_y, l1_in_x, l1_in_y, RF_W)
    ...
    if u4_out_x and rx1_in_x:
        add_track(board, FCU, "RX_LNA",
                  u4_out_x, u4_out_y, rx1_in_x, rx1_in_y, SIG_W)

    # TCXO output: X1 pin 3 → U1 XA (pin 8)
    x1_out_x, x1_out_y = pad_xy(x1, "3")
    u1_xa_x,  u1_xa_y  = pad_xy(u1, "8")
    if x1_out_x and u1_xa_x:
        add_track(board, FCU, "TCXO_OUT",
                  x1_out_x, x1_out_y, u1_xa_x, u1_xa_y, SIG_W)
    ...
    for xn, yn in ((130.5, 107.5), (130.5, 127.5),
                   (154.5, 107.5), (154.5, 127.5)):
    ...
    add_silk_label(board, attr, 108.5, 105.0, size=0.80, thickness=0.12,
