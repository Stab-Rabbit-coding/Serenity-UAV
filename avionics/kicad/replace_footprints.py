...
def replace_footprints(pcb_path, ref_map, out_path=None):
    ...
    for plan in plans:
        ...
        if plan["flipped"]:
            # pcbnew 9's Flip() takes a FLIP_DIRECTION enum; pylint (introspecting
            # whatever pcbnew is installed locally) may only see the older
            # KiCad 7/8 bool-argument signature.
            flip_dir = pcbnew.FLIP_DIRECTION_LEFT_RIGHT  # pylint: disable=no-member
            fp.Flip(fp.GetPosition(), flip_dir)
        replaced.append(
            f"{plan['ref']}: {plan['fp_name']} ({len(list(fp.Pads()))} pads)"
        )
