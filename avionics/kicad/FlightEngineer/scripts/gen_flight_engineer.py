#!/usr/bin/env python3
"""
gen_flight_engineer.py — Flight Engineer Power Distribution Board KiCad file generator
...
Usage:  python3 gen_flight_engineer.py   (run from any dir)
"""
...
def _uid(tag: str) -> str:
    return str(uuid.uuid5(_NS, f"Flight Engineer:{tag}"))
...
def _prop(name, val, x, y, rot=0, hide=False):
    ...
    return (
        f'    (property "{name}" "{val}" (at {x:.2f} {y:.2f} {rot})'
...
# Flight Engineer's battery/current telemetry is also TPM-signed and republished on
# both isolated buses, per "bake the trusted can+rs485 interface into flight_engineer
...
# sensors added) -- see FlightEngineer.md for details.
# ---------------------------------------------------------------------------
...
def gen_sch() -> str:
    ...
    # -----------------------------------------------------------------------
    parts.append(
        text_note("=== Section A: Battery Input / Main Bus Filter ===", 10, 15)
    )
    ...
    parts.append(
        sym_inst("M3_ChassisPad", "J_PGND_BATT", "M3 chassis lug", cx + 10, cy + 8)
    )
    ...
    cx, cy = 45, 30
    parts.append(
        sym_inst("CMC_7440640500", "CM1", "Wurth 7440640500 10A 2x100uH", cx, cy)
    )
    ...
    cx, cy = 70, 30
    parts.append(
        sym_inst("CMC_7440640500", "CM2", "Wurth 7440640500 10A 2x100uH", cx, cy)
    )
    ...
    cx, cy = 93, 30
    parts.append(
        sym_inst("Fuse_MAXI_150A", "F1", "Littelfuse 0297150.ZXNV 150A MAXI", cx, cy)
    )
    ...
    parts.append(
        sym_inst("C_Generic", "C_Y1", "Kemet SA305E472MAR 4.7nF/250V Y2", 58, 48)
    )
    parts.append(pwr_sym("VBAT", 58 - 2.54, 48, rot=0))
    parts.append(glabel("CHASSIS", 58 + 2.54, 48, rot=0))

    parts.append(
        sym_inst("C_Generic", "C_Y2", "Kemet SA305E472MAR 4.7nF/250V Y2", 65, 48)
    )
    ...
    # -----------------------------------------------------------------------
    parts.append(
        text_note("=== Section B: Battery Disconnect + Main Shunt ===", 100, 15)
    )
    ...
    parts.append(
        sym_inst(
            "R_Generic",
            "R_CHGND",
            "0R 0402 (chassis-PGND bond, configurable)",
            cx - 12,
            cy + 18,
        )
    )
    ...
    parts.append(
        sym_inst(
            "M3_ChassisPad", "J_CHASSIS", "M3 x8 brass standoff (x4)", cx - 5, cy + 25
        )
    )
    ...
    parts.append(
        sym_inst(
            "CSS2H_2512K_1mOhm", "RS_MAIN", "Bourns CSS2H-2512K-1L00F 1mOhm 3W", cx, cy
        )
    )
    parts.append(pwr_sym("VBAT", cx - 6.35, cy - 1.27, rot=0))
    parts.append(
        pwr_sym("PGND", cx - 6.35, cy + 1.27, rot=0)
    )  # F- carried from PGND rail
    ...
    cx, cy = 178, 30
    parts.append(
        sym_inst("INA226AIDGSR", "U_IS_MAIN", "INA226AIDGSR I2C addr=0x44", cx, cy)
    )
    ...
    def esc_branch(
        ...
        parts.append(
            sym_inst(
                "Fuse_Mini_40A",
                f_ref,
                "Littelfuse 0297040.WXNV 40A mini blade",
                fx,
                fy,
                dnp=dnp,
            )
        ...
        parts.append(
            sym_inst(
                "CSS2H_2512K_1mOhm",
                rs_ref,
                "Bourns CSS2H-2512K-1L00F 1mOhm 3W",
                cx2,
                cy2,
                dnp=dnp,
            )
        ...
        parts.append(
            sym_inst(
                "XT30PW_F",
                j_ref,
                "Amass XT30PW-F PCB-mount XT30 female",
                cx2,
                cy2,
                dnp=dnp,
            )
        ...
        parts.append(
            sym_inst(
                "M3_ChassisPad",
                shld_ref,
                "M3 chassis lug for ESC cable shield",
                bx + 18,
                base_y + 97,
                dnp=dnp,
            )
    ...
    for idx, info in enumerate(esc_info):
        n, f_ref, cdec_ref, cm_ref, rs_ref, uis_ref, j_ref, shld_ref, addr, a0, a1 = (
            info
        )
        esc_branch(
            n,
    ...
    def bec5v_half(suffix, bx, by):
        ...
        parts.append(
            sym_inst(
                "FB_Generic", f"FB_5V{suffix}", "Wurth 742792612 600R@100MHz 2A", bx, by
            )
        )
        ...
        parts.append(
            sym_inst(
                "C_Generic", f"C_BEC{suffix}_IN", "100uF/50V electrolytic", bx, by + 10
            )
        )
        ...
        parts.append(
            sym_inst(
                "L_Generic", f"L{suffix}", "Wurth 744314100 10uH >=6A Isat", bx + 15, by
            )
        )
        ...
        parts.append(
            sym_inst(
                "C_Generic",
                f"C_BEC{suffix}_OUT",
                "220uF 10V electrolytic + 100nF X7R",
                bx + 50,
                by + 10,
            )
        )
        ...
        parts.append(
            sym_inst(
                "MBRD1045CT",
                f"D_OR{suffix}",
                "MBRD1045CT Dual Schottky 10A/45V",
                bx + 60,
                by + 5,
            )
        )
        parts.append(
            glabel(f"BEC5V{suffix}_RAW", bx + 60 - 6.35, by + 5 - 2.54, rot=180)
        )
    ...
    parts.append(
        sym_inst("Nano_Fit_4P", "J_5V", "Molex Nano-Fit 4P 2.5mm avionics bus", 355, 52)
    )
    ...
    parts.append(
        sym_inst(
            "M3_ChassisPad",
            "J_SHLD_5V",
            "PGND via-pad 1.2mm for 5V cable shield",
            355,
            64,
        )
    )
    ...
    bx, by = 250, 130
    parts.append(
        sym_inst("FB_Generic", "FB_6V", "Wurth 742792612 600R@100MHz 2A", bx, by)
    )
    parts.append(pwr_sym("VDIS", bx - 2.54, by, rot=0))
    parts.append(glabel("FB6V_OUT", bx + 2.54, by, rot=0))

    parts.append(
        sym_inst("C_Generic", "C_BEC_SV_IN", "100uF/50V electrolytic", bx, by + 10)
    )
    parts.append(glabel("FB6V_OUT", bx - 2.54, by + 10, rot=180))
    parts.append(pwr_sym("PGND", bx + 2.54, by + 10, rot=180))

    parts.append(
        sym_inst("L_Generic", "L3", "Wurth 744314100 10uH >=6A Isat", bx + 15, by)
    )
    ...
    parts.append(
        sym_inst(
            "TPS54540DDAR",
            "U_BEC_6V",
            "TPS54540DDAR 5A/40V buck 6.0V servo rail",
            cx2,
            cy2,
        )
    )
    ...
    parts.append(
        sym_inst("R_Generic", "R_FB6_1", "R for Vout=6.0V (top)", bx + 20, by + 20)
    )
    parts.append(glabel("6V_SERVO", bx + 20 - 2.54, by + 20, rot=180))
    parts.append(glabel("FB6V_FBK", bx + 20 + 2.54, by + 20, rot=0))

    parts.append(
        sym_inst("R_Generic", "R_FB6_2", "R for Vout=6.0V (bot)", bx + 20, by + 30)
    )
    ...
    parts.append(
        sym_inst(
            "C_Generic", "C_BEC_SV_OUT", "100uF + 100nF output cap", bx + 50, by + 10
        )
    )
    ...
    parts.append(
        sym_inst(
            "Nano_Fit_4P", "J_6V", "Molex Nano-Fit 4P 2.5mm servo bus", bx + 65, by + 5
        )
    )
    jcx = bx + 65
    ...
    parts.append(
        sym_inst(
            "M3_ChassisPad",
            "J_SHLD_6V",
            "PGND via-pad for 6V cable shield",
            jcx,
            by + 20,
        )
    )
    ...
    jbx, jby = 400, 30
    parts.append(
        sym_inst("JST_XH_7P", "J_BAL", "JST XH-7P 2.54mm 6S balance lead", jbx, jby)
    )
    ...
    for i in range(1, 7):
        ...
        parts.append(
            sym_inst(
                "R_Generic",
                f"R_BAL{i}",
                "100R 0402 balance lead protection resistor",
                rx,
                ry,
                dnp=False,
            )
    ...
    parts.append(
        sym_inst(
            "BQ76930PWRQ1",
            "U_CELL",
            "BQ76930PWRQ1 6S cell monitor + HW protection",
            ucx,
            ucy,
        )
    )
    ...
    for i in range(1, 7):
        py_off = -15.24 + i * 2.54
        parts.append(
            glabel(f"VC{i}_R", ucx - 15.24, ucy + (-12.70 + (i - 1) * 2.54), rot=180)
        )
    ...
    parts.append(
        sym_inst("C_Generic", "C_CAP", "4.7uF X7R 0603 BQ76930 boot cap", 455, 95)
    )
    ...
    parts.append(
        sym_inst(
            "Conn_JST_GH_02P",
            "J_NTC",
            "JST-GH 2P 1.25mm battery NTC thermistor input",
            455,
            115,
        )
    )
    ...
    parts.append(
        sym_inst(
            "M3_ChassisPad", "J_SHLD_NTC", "PGND via-pad NTC cable shield", 455, 128
        )
    )
    ...
    parts.append(
        sym_inst("C_Generic", "C_NTC", "100nF X7R 0805 BQ76930 TS1 filter", 468, 115)
    )
    ...
    # -----------------------------------------------------------------------
    parts.append(
        text_note("=== Section G: I2C Interface, TVS, Alert, Connectors ===", 538, 15)
    )
    ...
    parts.append(
        sym_inst(
            "PRTR5V0U2X",
            "D_I2C",
            "NXP PRTR5V0U2X Dual TVS 5V clamp I2C RF protection",
            555,
            35,
        )
    )
    ...
    parts.append(
        sym_inst("R_Generic", "R_ALERT", "2.2k 0402 ALERT pull-up to +5V", 570, 50)
    )
    parts.append(pwr_sym("+5V", 570 - 2.54, 50, rot=0))
    parts.append(glabel("PDB_ALERT_N", 570 + 2.54, 50, rot=0))

    # J_I2C — JST-GH 4-pin to Pilot/Shepherd Bay A
    parts.append(
        sym_inst(
            "Conn_JST_GH_04P",
            "J_I2C",
            "JST-GH SM04B-GHS-TB 4P 1.25mm PDB I2C to FC1",
            590,
            38,
        )
    )
    ...
    parts.append(
        sym_inst(
            "M3_ChassisPad", "J_SHLD_I2C", "PGND via-pad I2C cable shield", 590, 52
        )
    )
    parts.append(pwr_sym("PGND", 590 - 2.54, 52, rot=0))

    # J_ALERT — JST-GH 2-pin ALERT output to Pilot GPIO
    parts.append(
        sym_inst(
            "Conn_JST_GH_02P",
            "J_ALERT",
            "JST-GH SM02B-GHS-TB 2P 1.25mm ALERT to Pilot GPIO",
            590,
            65,
        )
    )
    ...
    parts.append(
        sym_inst(
            "M3_ChassisPad", "J_SHLD_ALERT", "PGND via-pad ALERT cable shield", 590, 75
        )
    )
    ...
    # +3V3 buck fed from Flight Engineer's existing regulated +5V avionics rail
    ...
    sch = [
        ...
        '  (paper "A1")',
        "  (title_block",
        '    (title "Flight Engineer — EMI-Hardened Power Distribution Board")'
...
def gen_pcb() -> str:
    ...
    lines = [
        ...
        f'\t(gr_text "Flight Engineer Rev R" (at {W/2:.2f} {H + 2.5:.2f} 0) (layer "F.Fab")',
...
if __name__ == "__main__":
    ...
    # known, not-yet-fixed bug for Flight Engineer specifically ("HERE-relative script
    # paths need an extra .parent after the move; Flight Engineer's own gen scripts
