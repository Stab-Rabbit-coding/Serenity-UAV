...
# -*- coding: utf-8 -*-
"""
gen_commo_sch.py — Author Commo.kicad_sch from the as-placed Commo.kicad_pcb.
========================================================================
Schematic-first reconciliation of the Commo cape (2026-07-04).  The PCB layout
...
PCB = HERE.parent / "kicads" / "Commo.kicad_pcb"
OUT = HERE.parent / "kicads" / "Commo.kicad_sch"
...
def emit_instance(ref, value, X, Y, left, right, half_w, half_h):
    ...
    out.append(
        f'    (instances (project "Commo" (path "/{SHEET_UUID}" '
...
def main():
    ...
    hdr.append(
        '    (title "Commo — 49 MHz AX.25 + LoRa 915 MHz Transceiver Cape (Rev S1)")'
    )
    hdr.append('    (date "2026-07-04") (rev "S1") (company "Griffing Technology LLC")')
    hdr.append(
        '    (comment 1 "Schematic-first reconciliation: authored to match Commo.kicad_pcb (J1 dropped; UART on PB2 rails; PTT_N + RSSI_DCD on gated payload GPIOs)")'
    )
    hdr.append(
