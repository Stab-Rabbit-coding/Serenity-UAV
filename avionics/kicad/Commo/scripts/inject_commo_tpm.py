...
Commo (49 MHz + LoRa transceiver cape) has no MCU of its own -- it connects
only via the P1+P2 header stack to its host PocketBeagle2 node. The TPM
provides the last/first cryptographic signature for radio messages Commo
transmits and for messages it forwards to/from XO or elsewhere; binding it
to the PB2-I host it's plugged into (rather than giving it an isolated
identity of its own) is what lets Commo run as a self-sufficient cape in
...
slot already reserved on Commo's own P1/P2 trunk -- `SPI1_CS_TPM` /
...
elsewhere in `Commo.kicad_sch`. An earlier version of this script instead
...
for reusing *Pilot's* `SPI0_CLK` name in *Commo's* file (those are two
separate boards), but Commo's own P1/P2 block already reserves an SPI1 slot
named for exactly this purpose *within Commo's own file*, which does
...
Usage: python3 inject_commo_tpm.py
...
def _gen2pin(name):
    # Commo's own lib_symbols has no generic R/C (only fixed-value/pre-wired
...
def build():
    ...
    # Commo's TPM binds to the PB2-I host it's plugged into (not its own
    # standalone SPI peripheral): it provides the last/first cryptographic
    # signature for radio messages Commo transmits and for messages it
    # forwards to/from XO or elsewhere, so Commo can run as a self-sufficient
