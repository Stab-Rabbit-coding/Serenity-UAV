...
gen_emma_sch.py (user-locked decisions, 2026-07-04, TODO.md §1.2b):
...
PROVISIONAL / PENDING (flagged, TODO.md §1.2b — not fabrication-ready):
...
def main():
    ...
    jx, jy = ((pcbnew.ToMM(j1.GetPosition().x), pcbnew.ToMM(j1.GetPosition().y))
              if j1 is not None else (102.0, 117.5))
    ...
    hx, hy = 161.0, 108.0
    cmp_ = load_fp(board, "Package_TO_SOT_SMD", "SOT-23-5",
                   "RSSI_CMP", "LMV331 RSSI Carrier-Detect (part/pinout TBD)",
                   hx, hy)
    assign(board, cmp_, {"1": "RSSI_ANA", "2": "+3V3", "3": "RSSI_REF",
                         "4": "GND", "5": "RSSI_DCD"})
    rh = load_fp(board, "Resistor_SMD", "R_0402_1005Metric",
                 "RSSI_RH", "100k RSSI DCD Threshold Upper", hx, hy + 4.0)
    assign(board, rh, {"1": "+3V3", "2": "RSSI_REF"})
    rl = load_fp(board, "Resistor_SMD", "R_0402_1005Metric",
                 "RSSI_RL", "100k RSSI DCD Threshold Lower", hx, hy + 6.0)
    assign(board, rl, {"1": "RSSI_REF", "2": "GND"})
    cby = load_fp(board, "Capacitor_SMD", "C_0402_1005Metric",
                  "RSSI_CBY", "100nF RSSI_CMP VCC Bypass", hx, hy + 8.0)
    ...
    def near_j1(pt):
        return any(abs(pt.x - px) <= TOL and abs(pt.y - py) <= TOL
                   for px, py in j1_pads)
    dropped = 0
    for tr in list(board.GetTracks()):
        if isinstance(tr, pcbnew.PCB_VIA):
            if near_j1(tr.GetPosition()):
                board.Remove(tr); dropped += 1
