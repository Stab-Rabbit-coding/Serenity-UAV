...
Vera.md "EMI Hardening Status"): both Ethernet ports now carry the full magnetics + CMC +
TVS chain (Wurth 749010012A transformer, 2x Bourns SRF2012-100Y CMC, 2x Nexperia PRTR5V0U2X
TVS per port), and the CAN-FD transceiver is the galvanically-isolated TI ISOW1044BDFMR
(replacing the earlier non-isolated TCAN1042HG-Q1) with its own CMC+TVS on the bus side.
...
# ---------------------------------------------------------------------------
_NET_NAMES = [
    "", "+3V3", "+5V", "CAM_PWDN", "CAM_RESET_N", "CAM_SCL", "CAM_SDA",
    "CANFD_H", "CANFD_ISOGND", "CANFD_ISO_H", "CANFD_ISO_L", "CANFD_L",
    "CANFD_RX", "CANFD_STBY_N", "CANFD_TX",
    "CSI_CLK_N", "CSI_CLK_P", "CSI_D0_N", "CSI_D0_P",
    "ETH_IN_KSZ_RXN", "ETH_IN_KSZ_RXP", "ETH_IN_KSZ_TXN", "ETH_IN_KSZ_TXP",
    "ETH_IN_RXN", "ETH_IN_RXP", "ETH_IN_TXN", "ETH_IN_TXP",
    "ETH_IN_XFMR_RXN", "ETH_IN_XFMR_RXP", "ETH_IN_XFMR_TXN", "ETH_IN_XFMR_TXP",
    "ETH_OUT_KSZ_RXN", "ETH_OUT_KSZ_RXP", "ETH_OUT_KSZ_TXN", "ETH_OUT_KSZ_TXP",
    "ETH_OUT_RXN", "ETH_OUT_RXP", "ETH_OUT_TXN", "ETH_OUT_TXP",
    "ETH_OUT_XFMR_RXN", "ETH_OUT_XFMR_RXP", "ETH_OUT_XFMR_TXN", "ETH_OUT_XFMR_TXP",
    "GND", "LASER_CATHODE", "LASER_EN", "LASER_GATE", "LASER_IND", "LASER_KEY_IN",
    "MCU_NRST", "MCU_SWCLK", "MCU_SWDIO", "MDC", "MDIO", "PGND", "PGND_SHIELD",
    "PMIC_EN", "PMIC_SCL", "PMIC_SDA",
    "RGMII1_RXC", "RGMII1_RXCTL", "RGMII1_RXD0", "RGMII1_TXC", "RGMII1_TXCTL", "RGMII1_TXD0",
    "SD_CARD_NC", "SOC_BOOT0", "SOC_PORZ",
    "SW_1V2", "SW_INT_N", "SW_RESET_N", "SW_SPI_CS", "SW_SPI_MISO", "SW_SPI_MOSI",
    "SW_SPI_SCK", "SW_XTAL_25M",
    "TPM_PIRQ", "TPM_RESET_N", "TPM_SPI_CS", "TPM_SPI_MISO", "TPM_SPI_MOSI", "TPM_SPI_SCK",
    "UART_A2M", "UART_M2A", "UART_TOF_RX", "UART_TOF_TX",
    "VDD_CORE", "VDDR", "VDDSHV",
]
...
def _pad_smd(num, px, py, w, h, net, layers=None, side="F"):
    ...
    return (f'\t\t(pad "{num}" smd roundrect (at {px:.4f} {py:.4f}) (size {w:.4f} {h:.4f})'
...
def _pad_tht(num, px, py, w, h, drill, net, shape="circle"):
    ...
    if shape == "roundrect":
        ...
        shape_s, rr = "circle", ""
    return (f'\t\t(pad "{num}" thru_hole {shape_s} (at {px:.4f} {py:.4f}) (size {w:.4f} {h:.4f})'
            f' (drill {drill:.4f}) (layers "*.Cu" "*.Mask"){rr}{net_s} (uuid "{_uid()}"))')


def _pad_npth(px, py, drill):
    return (f'\t\t(pad "" np_thru_hole circle (at {px:.4f} {py:.4f}) (size {drill:.4f} {drill:.4f})'
            f' (drill {drill:.4f}) (layers "*.Cu" "*.Mask") (uuid "{_uid()}"))')


def _crtyd(x1, y1, x2, y2, layer=None, side="F"):
    layer = layer or f"{side}.CrtYd"
    return [f'\t\t(fp_rect (start {x1:.4f} {y1:.4f}) (end {x2:.4f} {y2:.4f})'
...
def _fab_text(txt, x, y, layer=None, size=1.0, side="F"):
    layer = layer or f"{side}.Fab"
    mirror = " (justify mirror)" if side == "B" else ""
    return [f'\t\t(fp_text user "{txt}" (at {x:.4f} {y:.4f}) (layer "{layer}")',
...
def comp_sot23(ref, value, x, y, g_net=NO_NET, d_net=NO_NET, s_net=NO_NET, angle=0.0, side="F"):
...
def comp_soic8(ref, value, x, y, nets=None, angle=0.0, side="F"):
    ...
    for num, py in pads_l:
        lines.append(_pad_smd(num, -2.475 * mx, py, 1.95, 0.60, nets.get(num, NO_NET), side=side))
...
def comp_soic16w(ref, value, x, y, nets=None, angle=0.0, side="F"):
    ...
    for num, py in pads_l:
        lines.append(_pad_smd(num, -4.25 * mx, py, 2.0, 0.6, nets.get(num, NO_NET), side=side))
    for num, py in pads_r:
        lines.append(_pad_smd(num, 4.25 * mx, py, 2.0, 0.6, nets.get(num, NO_NET), side=side))
    lines += _fp_end()
    return lines
...
def comp_eth_xfmr(ref, value, x, y, nets=None, angle=0.0, side="F"):
    ...
    for num, py in pads_l:
        lines.append(_pad_smd(num, -5.0 * mx, py, 1.8, 1.0, nets.get(num, NO_NET), side=side))
    for num, py in pads_r:
        lines.append(_pad_smd(num, 5.0 * mx, py, 1.8, 1.0, nets.get(num, NO_NET), side=side))
    lines += _fp_end()
    return lines
...
def comp_cmc_2012(ref, value, x, y, l1a_net=NO_NET, l1b_net=NO_NET, l2a_net=NO_NET,
                   l2b_net=NO_NET, angle=0.0, side="F"):
...
def comp_tvs_sot363(ref, value, x, y, a1_net=NO_NET, a2_net=NO_NET, k_net=NO_NET,
                     angle=0.0, side="F"):
...
def comp_jst_gh_4p(ref, value, x, y, p_nets=None):
    ...
    for i, px in enumerate(pads_x):
        lines.append(_pad_smd(str(i + 1), px, 1.95, 0.60, 1.70, p_nets[i] if i < len(p_nets) else NO_NET))
    lines += _fp_end()
    return lines
...
def comp_jst_gh_5p(ref, value, x, y, p_nets=None):
    ...
    for i, px in enumerate(pads_x):
        lines.append(_pad_smd(str(i + 1), px, 0.0, 0.6, 1.4, p_nets[i] if i < len(p_nets) else NO_NET))
    lines += _fp_end()
    return lines
...
def comp_2row_placeholder(ref, value, x, y, left, right, size, footprint_name, netmap,
                           side="F", pitch=2.54, pad_size=(2.2, 0.5)):
...
def rounded_board_outline(w, h, radius=3.0):
    ...
    def arc(x1, y1, mx, my, x2, y2):
        return [f"\t(gr_arc (start {x1:.4f} {y1:.4f}) (mid {mx:.4f} {my:.4f}) (end {x2:.4f} {y2:.4f})",
                '\t\t(stroke (width 0.05) (type solid)) (layer "Edge.Cuts")',
                f'\t\t(uuid "{_uid()}"))']
...
PMIC_NETMAP = {
    "VIN": "+5V", "GND": "GND", "EN": "PMIC_EN",
    "I2C_SDA": "PMIC_SDA", "I2C_SCL": "PMIC_SCL",
    "BUCK1_VDD_CORE": "VDD_CORE", "BUCK2_VDDSHV": "VDDSHV",
    "BUCK3_VDDR": "VDDR", "LDO1_3V3_CTRL": "+3V3",
}

AM62A_NETMAP = {
    "VDD_CORE": "VDD_CORE", "VDDSHV_MAIN": "VDDSHV", "VDDR_CORE": "VDDR", "GND": "GND",
    "PORz": "SOC_PORZ", "BOOT_MODE0": "SOC_BOOT0",
    "CAM_I2C_SDA": "CAM_SDA", "CAM_I2C_SCL": "CAM_SCL",
    "CAM_RESET_N": "CAM_RESET_N", "CAM_PWDN": "CAM_PWDN",
    "CSI_CLK_P": "CSI_CLK_P", "CSI_CLK_N": "CSI_CLK_N",
    "CSI_D0_P": "CSI_D0_P", "CSI_D0_N": "CSI_D0_N",
    "RGMII1_TXC": "RGMII1_TXC", "RGMII1_TXCTL": "RGMII1_TXCTL", "RGMII1_TXD0": "RGMII1_TXD0",
    "RGMII1_RXC": "RGMII1_RXC", "RGMII1_RXCTL": "RGMII1_RXCTL", "RGMII1_RXD0": "RGMII1_RXD0",
    "MDIO": "MDIO", "MDC": "MDC",
    "MCU_UART_TX": "UART_A2M", "MCU_UART_RX": "UART_M2A",
    "SD_CARD": "SD_CARD_NC",
}

MSPM0_NETMAP = {
    "+3V3": "+3V3", "GND": "GND",
    "NRST": "MCU_NRST", "SWCLK": "MCU_SWCLK", "SWDIO": "MCU_SWDIO",
    "MCAN_TX": "CANFD_TX", "MCAN_RX": "CANFD_RX",
    "UART0_TX": "UART_M2A", "UART0_RX": "UART_A2M",
    "UART1_TX": "UART_TOF_TX", "UART1_RX": "UART_TOF_RX",
    "SPI0_CS_TPM": "TPM_SPI_CS", "SPI0_SCK": "TPM_SPI_SCK",
    "SPI0_MOSI": "TPM_SPI_MOSI", "SPI0_MISO": "TPM_SPI_MISO",
    "TPM_PIRQ": "TPM_PIRQ",
    "SPI1_CS_SW": "SW_SPI_CS", "SPI1_SCK": "SW_SPI_SCK",
    "SPI1_MOSI": "SW_SPI_MOSI", "SPI1_MISO": "SW_SPI_MISO",
    "GPIO_LASER_EN": "LASER_EN", "GPIO_LASER_KEY": "LASER_KEY_IN", "GPIO_LASER_IND": "LASER_IND",
}

TPM_NETMAP = {
    "+3V3": "+3V3", "GND": "GND",
    "SPI_CS": "TPM_SPI_CS", "SPI_CLK": "TPM_SPI_SCK",
    "SPI_MOSI": "TPM_SPI_MOSI", "SPI_MISO": "TPM_SPI_MISO",
    "PIRQ": "TPM_PIRQ", "RESET_N": "TPM_RESET_N",
}

KSZ_NETMAP = {
    "+3V3": "+3V3", "+1V2": "SW_1V2", "GND": "GND",
    "RESET_N": "SW_RESET_N", "INT_N": "SW_INT_N", "XTAL_25M": "SW_XTAL_25M",
    "SPI_CS_HOST": "SW_SPI_CS", "SPI_SCK_HOST": "SW_SPI_SCK",
    "SPI_MOSI_HOST": "SW_SPI_MOSI", "SPI_MISO_HOST": "SW_SPI_MISO",
    "P1_RGMII_TXC": "RGMII1_TXC", "P1_RGMII_TXCTL": "RGMII1_TXCTL", "P1_RGMII_TXD0": "RGMII1_TXD0",
    "P1_RGMII_RXC": "RGMII1_RXC", "P1_RGMII_RXCTL": "RGMII1_RXCTL", "P1_RGMII_RXD0": "RGMII1_RXD0",
    "P2_TXP": "ETH_IN_KSZ_TXP", "P2_TXN": "ETH_IN_KSZ_TXN",
    "P2_RXP": "ETH_IN_KSZ_RXP", "P2_RXN": "ETH_IN_KSZ_RXN",
    "P3_TXP": "ETH_OUT_KSZ_TXP", "P3_TXN": "ETH_OUT_KSZ_TXN",
    "P3_RXP": "ETH_OUT_KSZ_RXP", "P3_RXN": "ETH_OUT_KSZ_RXN",
}

ISOW_NETS = {
    "1": N("GND"), "2": N("CANFD_TX"), "3": N("CANFD_STBY_N"), "4": N("CANFD_RX"),
    "6": N("+3V3"), "10": N("CANFD_ISOGND"), "11": N("+5V"),
    "14": N("CANFD_ISO_L"), "15": N("CANFD_ISO_H"),
}
...
def gen_pcb() -> str:
    ...
    lines += [
        ...
        f"\t\t(effects (font (size 1.5 1.5) (thickness 0.15)))", f'\t\t(uuid "{_uid()}"))',
        f'\t(gr_text "Griffing Technology LLC | CC BY 4.0" (at {W/2:.2f} {H + 5.5:.2f} 0) (layer "F.Fab")',
        f"\t\t(effects (font (size 1.0 1.0) (thickness 0.1)))", f'\t\t(uuid "{_uid()}"))',
    ]

    for i, (mx, my) in enumerate([(MH_M, MH_M), (W - MH_M, MH_M), (MH_M, H - MH_M), (W - MH_M, H - MH_M)]):
    ...
    lines += comp_2row_placeholder(
        "U1", "TI AM62A7", 24.0, 22.0, AM62A_L, AM62A_R, AM62A_SIZE,
        "Vera:BGA484_18x18mm_PLACEHOLDER", AM62A_NETMAP, pitch=1.27)

    lines += comp_jst_gh_4p("J_CAM1", "CSI_CLK/D0", 62.0, 10.0,
                             p_nets=[N("CSI_CLK_P"), N("CSI_CLK_N"), N("CSI_D0_P"), N("CSI_D0_N")])
    lines += comp_jst_gh_4p("J_CAM2", "CAM SDA/SCL/RST/3V3", 62.0, 22.0,
    ...
    lines += comp_2row_placeholder(
        "U_PMIC", "TPS65219", 12.0, 8.0, PMIC_L, PMIC_R, PMIC_SIZE,
        "Vera:PMIC_TPS65219_PLACEHOLDER_VQFN32", PMIC_NETMAP, side="B", pitch=1.0)

    lines += comp_2row_placeholder(
        "U5", "Infineon SLB9670", 34.0, 8.0, TPM_L, TPM_R, TPM_SIZE,
        "Vera:TPM_SLB9670_PLACEHOLDER_VQFN32", TPM_NETMAP, side="B", pitch=1.0)

    lines += comp_2row_placeholder(
        "U3", "TI MSPM0G3507", 12.0, 34.0, MSPM0_L, MSPM0_R, MSPM0_SIZE,
        "Vera:MSPM0G3507_PLACEHOLDER_VSSOP28", MSPM0_NETMAP, side="B", pitch=0.65,
        pad_size=(1.1, 0.3))
    lines += comp_0402("C_MCU1", "100nF", 12.0, 44.0, p1_net=N("+3V3"), p2_net=N("GND"), side="B")

    lines += comp_2row_placeholder(
        "U2", "Microchip KSZ9477", 36.0, 36.0, KSZ_L, KSZ_R, KSZ_SIZE,
        "Vera:QFN128_KSZ9477_PLACEHOLDER_14x14mm", KSZ_NETMAP, side="B", pitch=0.9)
    lines += comp_crystal_2p("Y1", "25MHz", 36.0, 50.0, p1_net=N("SW_XTAL_25M"), p2_net=N("GND"), side="B")
    ...
    def eth_port_chain(port, x0, y0, xfmr_ref, cmc_tx, cmc_rx, tvs_tx, tvs_rx, conn_ref):
        lines2 = []
        lines2 += comp_eth_xfmr(xfmr_ref, "Wurth 749010012A", x0, y0, nets={
            "1": N(f"{port}_KSZ_TXP"), "2": N(f"{port}_KSZ_TXN"),
            "3": N(f"{port}_KSZ_RXP"), "4": N(f"{port}_KSZ_RXN"),
            "5": N(f"{port}_XFMR_TXP"), "6": N(f"{port}_XFMR_TXN"),
            "7": N(f"{port}_XFMR_RXP"), "8": N(f"{port}_XFMR_RXN"),
        })
        lines2 += comp_cmc_2012(cmc_tx, "SRF2012-100Y (TX)", x0 + 8, y0 - 2,
                                 l1a_net=N(f"{port}_XFMR_TXP"), l1b_net=N(f"{port}_TXP"),
                                 l2a_net=N(f"{port}_XFMR_TXN"), l2b_net=N(f"{port}_TXN"))
        lines2 += comp_cmc_2012(cmc_rx, "SRF2012-100Y (RX)", x0 + 8, y0 + 2,
                                 l1a_net=N(f"{port}_XFMR_RXP"), l1b_net=N(f"{port}_RXP"),
                                 l2a_net=N(f"{port}_XFMR_RXN"), l2b_net=N(f"{port}_RXN"))
        lines2 += comp_tvs_sot363(tvs_tx, "PRTR5V0U2X (TX)", x0 + 14, y0 - 2,
                                   a1_net=N(f"{port}_TXP"), a2_net=N(f"{port}_TXN"), k_net=N("GND"))
        lines2 += comp_tvs_sot363(tvs_rx, "PRTR5V0U2X (RX)", x0 + 14, y0 + 2,
                                   a1_net=N(f"{port}_RXP"), a2_net=N(f"{port}_RXN"), k_net=N("GND"))
        lines2 += comp_jst_gh_5p(conn_ref, f"JST-GH 5P (shielded) Ethernet ring", x0 + 24, y0,
                                  p_nets=[N("PGND_SHIELD"), N(f"{port}_TXP"), N(f"{port}_TXN"),
                                          N(f"{port}_RXP"), N(f"{port}_RXN")])
        return lines2

    lines += eth_port_chain("ETH_IN", 46.0, 48.0, "T1", "CMC1", "CMC2", "D1", "D2", "J_ETH_IN")
    lines += eth_port_chain("ETH_OUT", 46.0, 60.0, "T2", "CMC3", "CMC4", "D3", "D4", "J_ETH_OUT")
    ...
    lines += comp_cmc_2012("CMC5", "SRF2012-100Y (CAN)", 20.0, 48.0,
                            l1a_net=N("CANFD_ISO_H"), l1b_net=N("CANFD_H"),
                            l2a_net=N("CANFD_ISO_L"), l2b_net=N("CANFD_L"))
    lines += comp_tvs_sot363("D5", "PRTR5V0U2X (CAN)", 26.0, 48.0,
                              a1_net=N("CANFD_H"), a2_net=N("CANFD_L"), k_net=N("PGND"))
    lines += comp_jst_gh_4p("J_CANFD", "5V/CANH/CANL/GND", 12.0, 60.0,
                             p_nets=[N("+5V"), N("CANFD_H"), N("CANFD_L"), N("GND")])

    # --- FRONT: power input + ToF + laser driver ---
    lines += comp_jst_gh_2p("J_PWR", "JST-GH 2P +5V/GND", 11.0, 70.0, p1_net=N("+5V"), p2_net=N("GND"))
    lines += comp_0402("J_CHASSIS_R", "0R chassis bond", 16.0, 70.0, p1_net=N("GND"), p2_net=N("PGND"))
    lines += comp_0402("C_ISO", "100nF/500V", 22.0, 70.0, p1_net=N("GND"), p2_net=N("PGND"))

    lines += comp_jst_gh_4p("J_TOF", "TFmini-S 5V/GND/TX/RX", 32.0, 70.0,
                             p_nets=[N("+5V"), N("GND"), N("UART_TOF_TX"), N("UART_TOF_RX")])

    lines += comp_sot23("Q1", "AO3400", 52.0, 70.0, g_net=N("LASER_GATE"), d_net=N("LASER_CATHODE"), s_net=N("GND"))
    lines += comp_0402("R1", "100R", 45.0, 70.0, p1_net=N("LASER_EN"), p2_net=N("LASER_GATE"))
    lines += comp_0402("R2", "10k", 45.0, 74.0, p1_net=N("LASER_GATE"), p2_net=N("GND"))
    lines += comp_jst_sh_2p("J_LASER", "laser +5V/cathode", 62.0, 70.0,
                             p1_net=N("+5V"), p2_net=N("LASER_CATHODE"))
