...
# ---------------------------------------------------------------------------
# Net registry — ordered to match the stub in gen_kaylee.py gen_pcb()
# ---------------------------------------------------------------------------
...
def _fp(lib_ref: str, ref: str, value: str,
        x: float, y: float, angle: float = 0.0) -> list[str]:
...
def _fp_tht(lib_ref: str, ref: str, value: str,
            x: float, y: float, angle: float = 0.0) -> list[str]:
...
def _pad_smd(num: str, px: float, py: float,
             w: float, h: float,
             net: tuple[int, str],
             layers: str = '"F.Cu" "F.Mask" "F.Paste"') -> str:
...
def _pad_tht(num: str, px: float, py: float,
...
def _crtyd(x1: float, y1: float, x2: float, y2: float,
           layer: str = "F.CrtYd") -> list[str]:
    """Courtyard rectangle."""
    return [
...
def _fab_rect(x1: float, y1: float, x2: float, y2: float,
              layer: str = "F.Fab") -> list[str]:
    return [
        f'\t\t(fp_rect (start {x1:.4f} {y1:.4f}) (end {x2:.4f} {y2:.4f})'
...
def _silk_circle(cx: float, cy: float, r: float,
                 layer: str = "F.SilkS") -> list[str]:
...
def comp_m3_lug(ref: str, x: float, y: float,
                net: tuple[int, str] = NO_NET) -> list[str]:
    """M3 chassis lug — plated THT with CHASSIS net."""
    lines = _fp_tht("MountingHole:MountingHole_3.2mm_M3_Pad_Via",
                    ref, "M3_Lug", x, y)
    lines += _crtyd(-3.5, -3.5, 3.5, 3.5)
    ni, nn = net
    lines.append(
...
def comp_0402(ref: str, value: str,
              x: float, y: float,
              p1_net: tuple[int, str] = NO_NET,
              p2_net: tuple[int, str] = NO_NET,
              angle: float = 0.0) -> list[str]:
...
def comp_0805(ref: str, value: str,
              x: float, y: float,
              p1_net: tuple[int, str] = NO_NET,
              p2_net: tuple[int, str] = NO_NET,
              angle: float = 0.0) -> list[str]:
...
def comp_1210(ref: str, value: str,
              x: float, y: float,
              p1_net: tuple[int, str] = NO_NET,
              p2_net: tuple[int, str] = NO_NET,
              angle: float = 0.0) -> list[str]:
...
def comp_smb_diode(ref: str, value: str,
                   x: float, y: float,
                   anode_net: tuple[int, str] = NO_NET,
                   cathode_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_sot363(ref: str, value: str,
                x: float, y: float,
                nets: dict[str, tuple[int, str]] | None = None) -> list[str]:
    ...
    pads = [
        ("1", -0.95, -0.65), ("2", -0.95,  0.00), ("3", -0.95,  0.65),
        ("4",  0.95,  0.65), ("5",  0.95,  0.00), ("6",  0.95, -0.65),
    ]
...
def comp_msop10(ref: str, value: str,
                x: float, y: float,
                nets: dict[str, tuple[int, str]] | None = None) -> list[str]:
...
def comp_soic8(ref: str, value: str,
               x: float, y: float,
...
def comp_tssop30(ref: str, value: str,
                 x: float, y: float,
                 nets: dict[str, tuple[int, str]] | None = None) -> list[str]:
...
def comp_vqfn20(ref: str, value: str,
                x: float, y: float,
                nets: dict[str, tuple[int, str]] | None = None) -> list[str]:
    ...
    for i in range(5):
        num = str(i + 1)
        py = -1.0 + i * 0.5
        lines.append(_pad_smd(num, -1.9625, py, 0.825, 0.25,
                               nets.get(num, NO_NET)))
    # Pins 6-10 bottom
    for i in range(5):
        num = str(i + 6)
        px = 1.0 - i * 0.5
        lines.append(_pad_smd(num, px, 1.9625, 0.25, 0.825,
                               nets.get(num, NO_NET)))
...
def comp_dpak(ref: str, value: str,
              x: float, y: float,
              p1_net: tuple[int, str] = NO_NET,
              p2_net: tuple[int, str] = NO_NET,
              p3_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_xt60pw_f(ref: str, value: str,
                  x: float, y: float,
                  p_net: tuple[int, str] = NO_NET,
                  n_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_xt30pw_f(ref: str, value: str,
                  x: float, y: float,
                  p_net: tuple[int, str] = NO_NET,
                  n_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_cp_d10(ref: str, value: str,
                x: float, y: float,
                p_net: tuple[int, str] = NO_NET,
                n_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_y2_cap(ref: str, value: str,
                x: float, y: float,
                p1_net: tuple[int, str] = NO_NET,
                p2_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_nanofit_4p(ref: str, value: str,
                    x: float, y: float,
                    p_nets: list[tuple[int, str]] | None = None) -> list[str]:
...
def comp_jst_gh_2p(ref: str, value: str,
                   x: float, y: float,
                   p1_net: tuple[int, str] = NO_NET,
                   p2_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_jst_gh_4p(ref: str, value: str,
                   x: float, y: float,
                   p_nets: list[tuple[int, str]] | None = None) -> list[str]:
...
def comp_jst_xh_7p(ref: str, value: str,
                   x: float, y: float,
                   p_nets: list[tuple[int, str]] | None = None) -> list[str]:
...
def comp_wurth_cmc(ref: str, x: float, y: float,
    ...
    """
    lines = _fp_tht("Kaylee:CMC_WE-SL5_7440640500", ref,
                    "WE-SL5_7440640500", x, y)
...
def comp_maxi_fuse(ref: str, x: float, y: float,
                   p1_net: tuple[int, str] = NO_NET,
                   p2_net: tuple[int, str] = NO_NET) -> list[str]:
    ...
    """
    lines = _fp_tht("Kaylee:FuseHolder_MAXI_PCB", ref,
                    "FuseHolder_MAXI_150A", x, y)
...
def comp_mini_fuse(ref: str, x: float, y: float,
                   p1_net: tuple[int, str] = NO_NET,
                   p2_net: tuple[int, str] = NO_NET) -> list[str]:
...
def comp_kelvin_shunt(ref: str, x: float, y: float,
    ...
    """
    lines = _fp("Kaylee:R_Shunt_CSS2H_2512K", ref,
                "1mOhm_Kelvin_2512", x, y)
...
def _zone(net: tuple[int, str], layer: str,
          x1: float, y1: float, x2: float, y2: float,
          clearance: float = 0.5) -> list[str]:
...
def gen_pcb() -> str:
    ...
    for i, (mx, my) in enumerate([
        (MH_M, MH_M), (W - MH_M, MH_M),
        (MH_M, H - MH_M), (W - MH_M, H - MH_M),
    ]):
    ...
    lines += comp_xt60pw_f("J_BATT", "XT60PW-F", 13.0, 19.0,
                            p_net=N("VDIS"), n_net=N("PGND"))

    # CM1, CM2 — Wurth 7440640500 WE-SL5 CMC
    # Pin assignments: 1=VDIS_in, 2=VDIS_out, 3=PGND, 4=PGND
    lines += comp_wurth_cmc("CM1", 31.0, 12.0,
                             p1_net=N("VDIS"), p2_net=N("VDIS"),
                             p3_net=N("PGND"), p4_net=N("PGND"))
    lines += comp_wurth_cmc("CM2", 44.0, 12.0,
                             p1_net=N("VDIS"), p2_net=N("VDIS"),
                             p3_net=N("PGND"), p4_net=N("PGND"))

    # F1 — MAXI fuse holder (150 A)
    lines += comp_maxi_fuse("F1", 56.0, 9.0,
                             p1_net=N("VDIS"), p2_net=N("VDIS"))
    ...
    lines += comp_smb_diode("D1", "SMBJ33CA", 14.0, 24.0,
                             anode_net=N("PGND"), cathode_net=N("VDIS"))

    # C_Y1, C_Y2 — 4.7nF Y2 safety caps (VDIS-to-CHASSIS and PGND-to-CHASSIS)
    lines += comp_y2_cap("C_Y1", "4.7nF/250V_Y2", 22.0, 24.0,
                          p1_net=N("VDIS"), p2_net=N("CHASSIS"))
    lines += comp_y2_cap("C_Y2", "4.7nF/250V_Y2", 29.0, 24.0,
    ...
    lines += comp_0805("C3", "100nF/50V", 44.0, 24.0,
                        p1_net=N("VDIS"), p2_net=N("PGND"))
    ...
    lines += comp_soic8("Q_BATT_DSG", "AON6556", 16.0, 33.0, nets={
        "1": N("VBAT"),  "2": N("VBAT"),  "3": N("VBAT"),  "4": N("VBAT"),
        "5": N("VDIS"),  "6": N("VDIS"),  "7": N("VDIS"),  "8": N("DSG_GATE"),
    })

    # R_DSG_G — 10k 0402 gate pull-down
    lines += comp_0402("R_DSG_G", "10k", 25.0, 33.0,
                        p1_net=N("DSG_GATE"), p2_net=N("PGND"))

    # R_CHGND — 0Ω chassis bond (connects digital PGND to chassis)
    lines += comp_0402("R_CHGND", "0R", 30.5, 33.0,
                        p1_net=N("PGND"), p2_net=N("CHASSIS"))

    # RS_MAIN — 1mΩ Kelvin shunt (CSS2H-2512K)
    # C_in=VBAT, V+=MAIN_SNS_P, V-=MAIN_SNS_N, C_out=VBAT (return side not defined)
    lines += comp_kelvin_shunt("RS_MAIN", 39.0, 33.0,
                                c_in_net=N("VBAT"), s_p_net=N("MAIN_SNS_P"),
                                s_n_net=N("MAIN_SNS_N"), c_out_net=N("VBAT"))
    ...
    lines += comp_msop10("U_IS_MAIN", "INA226_0x44", 49.0, 33.0, nets={
        "1": N("MAIN_SNS_P"), "2": N("MAIN_SNS_N"), "3": N("PGND"),
        "4": N("+5V"), "5": N("PDB_SDA"), "6": N("PDB_SCL"),
        "7": N("PDB_ALERT_N"), "8": NO_NET, "9": NO_NET, "10": N("VBAT"),
    })
    ...
    esc_rs_out = [N("ESC1_RS_OUT"), N("ESC2_RS_OUT"),
                  N("ESC3_RS_OUT"), N("ESC4_RS_OUT")]
    esc_fused  = [N("ESC1_FUSED"), N("ESC2_FUSED"),
                  N("ESC3_FUSED"), N("ESC4_FUSED")]
    esc_sns_p  = [N("ESC1_SNS_P"), N("ESC2_SNS_P"),
                  N("ESC3_SNS_P"), N("ESC4_SNS_P")]
    esc_sns_n  = [N("ESC1_SNS_N"), N("ESC2_SNS_N"),
                  N("ESC3_SNS_N"), N("ESC4_SNS_N")]
    ...
    for n, cx in enumerate(esc_cx, start=1):
        ...
        lines += comp_mini_fuse(f"F_ESC{n}", cx - 5.0, 40.5,
                                 p1_net=N("VBAT"), p2_net=fsd)

        # RS_n — Kelvin shunt, centred at (cx, 47)
        lines += comp_kelvin_shunt(f"RS_{n}", cx, 47.0,
                                    c_in_net=fsd, s_p_net=snp,
                                    s_n_net=snn, c_out_net=rso)
        ...
        lines += comp_xt30pw_f(f"J_ESC{n}",
                                f"XT30PW-F_ESC{n}", cx, 61.0,
                                p_net=rso, n_net=N("PGND"))
    ...
    lines += comp_cp_d10("C_BEC1_IN", "100uF/50V", 69.0, 8.0,
    ...
    lines += comp_vqfn20("U_BEC_5V_1", "TPS54620RGYT", 84.0, 11.0, nets={
        ...
        "14": NO_NET, "15": N("BEC1_VIN_F"), "16": N("BEC1_VIN_F"),
        "17": N("BEC1_VIN_F"), "18": N("BEC1_VIN_F"), "19": N("BEC1_VIN_F"),
        "20": N("BEC1_VIN_F"), "21": N("PGND"),
    })
    ...
    # BEC2 ----------------------------------------------------------
    # FB_5V2
    lines += comp_0805("FB_5V2", "WE-742792612", 63.0, 28.0,
                        p1_net=N("VDIS"), p2_net=N("BEC2_VIN_F"))
    # C_BEC2_IN
    ...
    lines += comp_vqfn20("U_BEC_5V_2", "TPS54620RGYT", 84.0, 31.0, nets={
        ...
        "14": NO_NET, "15": N("BEC2_VIN_F"), "16": N("BEC2_VIN_F"),
    ...
    # FB_6V — ferrite 0805
    lines += comp_0805("FB_6V", "WE-742792612", 63.0, 48.0,
                        p1_net=N("VDIS"), p2_net=N("BEC6V_VIN_F"))
    ...
    lines += comp_soic8("U_BEC_6V", "TPS54540DDAR", 84.0, 49.5, nets={
        "1": NO_NET, "2": N("BEC6V_VIN_F"), "3": N("BEC6V_VIN_F"),
        "4": N("BEC6V_VIN_F"), "5": NO_NET,
        "6": N("+6V"), "7": NO_NET, "8": N("BEC6V_SW"),
    })
    ...
    lines += comp_sot363("D_I2C", "PRTR5V0U2X", 59.0, 54.0, nets={
        "1": N("PGND"),       "2": N("PDB_SDA_RAW"), "3": N("PDB_SCL_RAW"),
        "4": N("PDB_SCL"),   "5": N("PDB_SDA"),     "6": N("+5V"),
    })
