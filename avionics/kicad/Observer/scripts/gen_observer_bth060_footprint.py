...
Pad names A1..A60/B1..B60/C1..C60/D1..D60 match Observer_SoM_PCM071.kicad_sym so the
...
OUT = (
    ...
    / "Observer.pretty"
    / "phyCORE-AM62x_PCM071_2xBTH-060.kicad_mod"
)
...
def pad_smd(name, x, y):
    return (
        f'  (pad "{name}" smd roundrect (at {x:.4f} {y:.4f}) '
...
def npth(x, y, d):
    return (
        f'  (pad "" np_thru_hole circle (at {x:.4f} {y:.4f}) (size {d} {d}) '
...
def mhole(name, x, y):
    return (
        f'  (pad "{name}" np_thru_hole circle (at {x:.4f} {y:.4f}) '
...
def main():
    L = [
        '(footprint "phyCORE-AM62x_PCM071_2xBTH-060" (version 20240108) '
        ...
        'A1..D60 match Observer_SoM_PCM071. Verify row<->column + pin-1 vs Fig 6.")',
        "  (attr smd)",
