...
# -*- coding: utf-8 -*-
"""
gen_Observer_carrier_sch.py -- author Observer.kicad_sch in the SoM end-state.
=============================================================================
...
primary datasheets/manual:
  ...
  U_SOM  Observer_SoM_PCM071      PHYTEC phyCORE-AM62x PCM-071 (240-pad, 2xBTH-060)
  ...
  U5     SLB9672_TPM     Infineon SLB9672 TPM 2.0 (32-UQFN)
...
REAL_SYMS = {
    "SOM": ("Observer_SoM_PCM071", "Observer:phyCORE-AM62x_PCM071_2xBTH-060"),
    "KSZ": ("Observer_KSZ9477", "Observer:TQFP-128-1EP_KSZ9477_14x14mm_P0.4mm_EP10x10"),
    "MSP": (
...
# in Pilot.kicad_sch / XO.kicad_sch has WRONG pin numbers (a pin 17 on a
...
# also removes Observer's own "ISO_DCDC_1W_GENERIC ... MPN TBD" open item.
...
def build_msp_netmap(pins):
    ...
    GPIO_NET = {
        ...
        # Class 2 optic makes them optional defense-in-depth, OBSERVER_LASER_ANALYSIS)
        "PA22": "LASER_EN",
        # UART2 -> ADM2795E isolated RS-485 (added 2026-07-26, see Section H:
        # "so that all nodes have at least [CAN-FD + RS-485], and everyone
        # gets a TPM" -- Observer already had MCU+TPM+CAN-FD, this closes the gap).
...
def gen_sch():
    ...
    # "add rs485 to observer as well, so that all nodes have at least those
    # two busses" -- Observer already had MCU (U3) + TPM (U5) + isolated CAN-FD
    ...
    sch = [
        ...
        '    (comment 4 "SoM carrier: real clean-room symbols; see gen_Observer_carrier_sch.py")',
        "  )",
    ]
    sch.extend(parts)
    sch.append(")")
...
# Footprint each ref carries on the PCB (mirrors gen_Observer_carrier_pcb.py) so
