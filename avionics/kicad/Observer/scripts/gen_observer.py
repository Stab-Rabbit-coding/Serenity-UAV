...
IMPORTANT — verification status (do not treat as fab-ready without addressing these):
    ...
    - EMI hardening chain (added 2026-07-03, matching Pilot/XO Rev R baseline) uses REAL,
      ...
      SOIC-16W footprint, same part Pilot/XO use). These are NOT placeholders.
...
# ---------------------------------------------------------------------------
# kicad_pro  (adapted from gen_flight_engineer.py gen_pro(); Observer-specific net classes)
# ---------------------------------------------------------------------------
def gen_pro() -> str:
    import json
...
# ---------------------------------------------------------------------------
# kicad_sch low-level helpers (same conventions as gen_flight_engineer.py)
# ---------------------------------------------------------------------------
...
def lib_symbol_srf2012():
    ...
    Reused verbatim (pin layout) from this project's own verified Pilot/XO
...
def lib_symbol_prtr5v0u2x():
    ...
    from this project's own verified Pilot/XO generator (gen_cape_a2.py
...
# symbols/footprints from gen_cape_a2.py (Pilot) rather than re-deriving new
...
# non-isolated TCAN1042HG-Q1 to match Pilot/XO's CAN-FD isolation standard.
ISOW_SIZE = (3.75, 5.15)  # SOIC-16W_7.5x10.3mm — real, from Pilot's own footprint
ISOW_L = [
    ("GND1", "1", "power_in"),
    ("TXD", "2", "input"),
    ("STB_N", "3", "input"),
...
# 8-pin part already used on Pilot/XO (TODO.md's "HX1188NL" reference for this
...
def gen_sch() -> str:
    ...
    parts.append(
        text_note(
            "OBSERVER — Nose/Cargo-Bay Vision, ToF & Laser Board  |  STANDALONE PCB, NOT a PB2-I cape",
            10,
            8,
            2.0,
        )
    ...
    parts.append(
        text_note(
            ...
            "physical microSD on Observer (logging lives on Pilot/XO per CLAUDE.md).",
            cx - 7,
            cy + 30,
            0.9,
        )
    ...
    # =====================================================================
    # EMI hardening — Ethernet ports (matches Pilot/XO Rev R baseline):
    # KSZ9477 port -> Wurth 749010012A magnetics -> SRF2012-100Y CMC (x2,
    # one per differential pair) -> PRTR5V0U2X TVS (x2, shunt) -> connector.
    # =====================================================================
    parts.append(
        text_note(
            "EMI hardening chain per Ethernet port (matches Pilot/XO Rev R baseline):\n"
    ...
    def eth_port_chain(
        ...
        # Connector: JST-GH 5P (GND, TX+, TX-, RX+, RX-) — matches Pilot's real
    ...
    # insulation) replaces the non-isolated TCAN1042HG-Q1 to match Pilot/XO's
    # CAN-FD isolation standard [REF-IEC-001, REF-VDE-001]. + SRF2012-100Y CMC
    # + PRTR5V0U2X TVS on the bus side, matching Pilot's real CAN chain.
    # =====================================================================
    cx, cy = 40, 235
    parts.append(
