...
FR4, rounded corners (3mm radius, matching Pilot/Flight Engineer convention).

EMI HARDENING (added 2026-07-03, matching Pilot/XO Rev R baseline — see TODO.md §1.2c and
...
All of these reuse REAL footprints already verified in this project's own Pilot/XO generator
...
Usage:  python3 gen_jayne_pcb.py
"""
...
# ---------------------------------------------------------------------------
# Low-level S-expression helpers (same conventions as gen_flight_engineer_pcb.py)
# ---------------------------------------------------------------------------
...
def comp_soic16w(ref, value, x, y, nets=None, angle=0.0, side="F"):
    ...
    (same part + footprint already verified/working on Pilot/XO)."""
    nets = nets or {}
...
def comp_eth_xfmr(ref, value, x, y, nets=None, angle=0.0, side="F"):
    ...
    already used on Pilot/XO's real Ethernet EMI-hardening chain."""
    nets = nets or {}
...
def comp_jst_gh_5p(ref, value, x, y, p_nets=None):
    ...
    — the REAL footprint used on Pilot for its Ethernet ports (5-pin: GND + 4
...
def comp_2row_placeholder(
    ...
    glabel_pin()/pwr_pin() calls in gen_jayne.py for this component — deliberately
...
def rounded_board_outline(w, h, radius=3.0):
    """4 straight Edge.Cuts segments + 4 corner arcs, matching Pilot/Flight Engineer's
...
# ---------------------------------------------------------------------------
# Pin lists — copied from gen_jayne.py (kept in sync manually; both files are
...
# glabel_pin()/pwr_pin() calls in gen_jayne.py's gen_sch(). Deliberately
...
def gen_pcb() -> str:
    ...
    # All FRONT side. Matches Pilot/XO Rev R baseline.
    # =====================================================================
