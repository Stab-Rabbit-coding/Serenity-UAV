#!/usr/bin/env python3
"""inject_kaylee_trust_module.py -- add the Section H trust module (MCU + TPM
+ isolated CAN-FD + isolated RS-485) into the EXISTING, working Kaylee.kicad_sch
via text injection, instead of a full gen_kaylee.py regeneration.

WHY injection, not regeneration: running gen_kaylee.py fresh from its own
current source (even with NONE of the Section H edits applied) does not
reproduce the checked-in Kaylee.kicad_sch -- it lands at 247 ERC errors
(all pre-existing, all in Sections A-G, none related to this change) where
the checked-in file has 0. That means gen_kaylee.py has drifted out of sync
...
Regenerating from gen_kaylee.py's gen_sch() would silently reintroduce that
...
leaving everything else byte-identical. gen_kaylee.py itself still carries
...
Usage: python3 inject_kaylee_trust_module.py
Output: overwrites avionics/kicad/Kaylee/kicads/Kaylee.kicad_sch in place.
...
SCH = KICADS / "Kaylee.kicad_sch"

sys.path.insert(0, str(HERE))
import gen_kaylee as gk  # noqa: E402
...
def two_pin_trust(lib_id, ref, value, cx, cy, net1, net2):
    ...
    (Kaylee's OWN lib_symbol_2pin_h -- NOT the same offset as this project's
    other generators' 2pin_h helper, which uses 3.81; verified directly
    against gen_kaylee.py's own _pin_def calls). Computed, not hand-typed,
