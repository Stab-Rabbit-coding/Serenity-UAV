#!/usr/bin/env python3
"""
add_eth_phy.py — Add one EM-hardened Ethernet port to Wash and Zoë.
...
For Zoë also removes orphaned PHY-side global_labels left by gen_cape_b2.py.

Usage:
    python3 add_eth_phy.py Wash.kicad_sch
    python3 add_eth_phy.py Zoë.kicad_sch
...
# Orphaned PHY-side labels in Zoë (left by gen_cape_b2.py).
...
def transform(path: str) -> None:
    ...
    is_cape_b = 'Zoë' in path or 'cape_b2' in path.lower()
    ...
    # -----------------------------------------------------------------------
    # Step 3: For Zoë, remove orphaned PHY-side labels.
    # -----------------------------------------------------------------------
    if is_cape_b:
