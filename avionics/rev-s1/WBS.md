# Serenity UAV — Avionics Rev S1 Redesign (COMMO / Zoe / FlightEngineer) Work Breakdown Structure (Detail)
...
## §1.2b — PCB Redesigns: COMMO / Zoe / FlightEngineer Rev S1
*(root `WBS.md` §1.2b)*
...
- [ ] **COMMO Rev S1 — add LoRa, replace JST with P1+P2 socket rails**
    ...
    - Replace JST GH 6P connector with 2× 20-pin 2.54 mm socket rails (P1 + P2),
        matching TACCO passthrough rail pinout so COMMO stacks cleanly on top.
    ...
    - [x] **Traces and nets regenerated and DRC issues resolved — partial (2026-06-20).**
        ...
        - [ ] **`GND2_ETH`/`VCC2_ETH` deferred — confirmed isolated domain (2026-06-20).**
            ...
            a second Ethernet port for the TACCO stack (matching Wash's 2-PHY config) and so COMMO
...
- [ ] **TACCO (Cape-B-2) Rev S1 — remove LoRa, add P1+P2 passthrough rails**
    - [x] **Schematic load-blocking bug FIXED 2026-07-04.** `TACCO.kicad_sch` would not open
        ...
        file. Converted them to a single valid `(text …)` annotation. TACCO now loads and ERC
        runs (564 pre-existing violations — the separate TACCO ERC backlog, not caused by this).
    - [x] **Real sch↔pcb divergence characterized 2026-07-04 — deeper than "still has
        LoRa".** `TACCO.kicad_pcb` is already at the intended end-state: **LoRa (RFM95W)
    ...
    - [x] **P1/P2 socket-rail footprints already placed (verified 2026-07-04).**
        `TACCO.kicad_pcb` already has real `2x18_P1_Socket`/`2x18_P2_Socket` footprints —
        placed via the same `complete_xcvr_49mhz2.py`-style direct-PCB-edit approach used
        on COMMO, not via the schematic. `TACCO.kicad_sch` still has a real `RFM95W` (LoRa)
    ...
    - Carry all PB2 P1+P2 signals from lower pins to upper sockets; add 0 Ω options
        ...
        passed through.
    - Update silk/fab: "Cape-B-2 Rev S1 — TACCO CN cape"; note COMMO header on upper face.
    - Run DRC → zero errors; generate gerbers to `avionics/kicad/gerbers/CAPE-B-2-S1/`.
    - **BLOCKS TACCO + COMMO fabrication order.**
