# Serenity UAV — Avionics Rev S1 Redesign (Commo / XO / Flight Engineer) Work Breakdown Structure (Detail)
...
- [ ] **Commo Rev S1 — add LoRa, replace JST with P1+P2 socket rails**
    ...
    - Replace JST GH 6P connector with 2× 20-pin 2.54 mm socket rails (P1 + P2),
        matching XO passthrough rail pinout so Commo stacks cleanly on top.
    - Update SRF2012-100Y CMC + TVS guard to cover LoRa SPI and antenna lines.
    - Update silk/fab layer: "Commo Rev S1 — 49 MHz AX.25 + LoRa 915 MHz".
    - Fitted in: River's Room, Simon's Medbay only (2 boards total).
    - Run DRC → zero errors; generate gerbers to `avionics/kicad/gerbers/Commo-S1/`.
    - **BLOCKS Commo fabrication order.**
    - [ ] **Schematic-first reconciliation — DECISIONS LOCKED 2026-07-04 (user).**
        Resolves the PCB-ahead-of-schematic gap by authoring `Commo.kicad_sch` to be the
        ...
        - **PTT_N → repurposed PB2-P2 payload GPIO,** presence-gated by a cape-detect
            ...
            GPIO only when Commo is detected; otherwise it keeps its `SERVO*`/payload
            function). Commo has no on-board MCU, so the host must key the T/R switch; the
        ...
        - [x] **Author `Commo.kicad_sch` — DONE 2026-07-04.** Generated by
            `avionics/kicad/gen_commo_sch.py` from the as-placed PCB (one black-box symbol
            ...
            signals Commo does not tap + 3 pre-existing in-circuit stubs — see below).
            Old EMI-stub schematic saved as `Commo.kicad_sch.stub-bak`.
        - [x] **PCB rework — DONE 2026-07-04** via `avionics/kicad/mod_commo_pcb.py`
        ...
        - [x] **DRC hard-violation debt CLEARED 2026-07-04 — CI validator PASSES.**
            ...
            both `Commo.kicad_sch` (ERC) and `Commo.kicad_pcb` (DRC). Editing the PCB pulled it
            ...
            `avionics/kicad/cleanup_commo_drc.py` and the schematic/PCB net work:
            - **26 net_conflict** (schematic-parity): schematic now uses **global labels**
                ...
                `mod_commo_pcb.py` assigns net-less pads the canonical `unconnected-(REF-PadN)`
            ...
            - **15 clearance**: POWER netclass clearance **0.20 → 0.10 mm** in `Commo.kicad_pro`
        ...
        - [x] **RSSI sub-circuit routed — DONE 2026-07-05** via `avionics/kicad/route_commo_rssi.py`
        ...
        - [ ] **RSSI_DCD still to route (1 net).** RSSI_CMP.5 → PB2-P2 pad 2 (105.4,130.2) is a
            ...
            (GUI push-shove) or with a real autorouter — `route_commo_rssi.py dcd` holds a
        ...
        - [ ] **PTT_N / RSSI_DCD presence-gated pinmux — firmware sign-off owed.** PB2-P2
            pin 1 (SERVO7 ball) / pin 2 (SERVO6 ball) are repurposed only when an Commo
    ...
    - [x] Components added to Commo board.
    - [x] Commo Kicad files renamed from XCVR to Commo
    ...
    - [x] **Traces and nets regenerated and DRC issues resolved — partial (2026-06-20).**
        ...
        - [ ] **`GND2_ETH`/`VCC2_ETH` deferred — confirmed isolated domain (2026-06-20).**
            Per user: ETH-PHY/T-ETH/PB2-P1/P2 were intentionally added to Commo so it can provide
            a second Ethernet port for the XO stack (matching Pilot's 2-PHY config) and so Commo
    ...
    - [ ] **Silk overlap/over-copper cleanup — NOT done, 174 pre-existing warnings.**
        ...
        since before this session and unchanged by it. Commo.md §10 documents a prior partial
...
- [ ] **XO (Cape-B-2) Rev S1 — remove LoRa, add P1+P2 passthrough rails**
    - [x] **Schematic load-blocking bug FIXED 2026-07-04.** `XO.kicad_sch` would not open
        ...
        file. Converted them to a single valid `(text …)` annotation. XO now loads and ERC
        runs (564 pre-existing violations — the separate XO ERC backlog, not caused by this).
    - [x] **Real sch↔pcb divergence characterized 2026-07-04 — deeper than "still has
        LoRa".** `XO.kicad_pcb` is already at the intended end-state: **LoRa (RFM95W)
        ...
        **`J_XCVR`** JST-GH Commo-cable connector (must disappear for the same reason Commo's J1
        did — Commo stacks via P1/P2 now), and an **SBUS block** (`U_SBUS_B`, `R_SBUS_RX`,
    ...
    - [ ] **Full sch↔pcb reference-designator remap — REFERRED TO USER (flight-hardware
        ...
        passthrough (TOP) sockets to the schematic, and re-run ERC. Commo's reconciliation
        (above) is the template; XO differs only in that its schematic was hand-authored and
        its refs don't already match the PCB the way Commo's did.
    ...
    - [x] **P1/P2 socket-rail footprints already placed (verified 2026-07-04).**
        `XO.kicad_pcb` already has real `2x18_P1_Socket`/`2x18_P2_Socket` footprints —
        placed via the same `complete_xcvr_49mhz2.py`-style direct-PCB-edit approach used
        on Commo, not via the schematic. `XO.kicad_sch` still has a real `RFM95W` (LoRa)
        symbol and the JST-GH connectors — schematic has NOT been updated to either add
        the P1/P2 headers or remove LoRa. Same PCB-ahead-of-schematic gap as Commo; see
...
- [ ] **Flight Engineer Rev S1 — remove 6 V BEC, add 5 V servo output**
    ...
    - **BLOCKS Flight Engineer fabrication order.**
...
carried forward, but layout, ERC/DRC, and production-file work happen on the Commo KiCad project
(`avionics/kicad/Commo.kicad_sch`/`.kicad_pcb`), now archived at `avionics/kicad/archive/`
...
gerber/BOM export for this stub were never started and carry forward instead as the Commo
