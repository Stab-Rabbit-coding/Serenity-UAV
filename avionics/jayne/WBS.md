...
P1+P2 header stack and does not mount onto a Wash/TACCO node; it is its own independent board
...
`avionics/kicad/Observer/scripts/gen_Jayne_pcb.py` (PCB), following the same script-then-manual-placement
pattern already used for FlightEngineer/Wash/TACCO (`gen_kaylee.py`/`gen_kaylee_pcb.py` etc.). This is a
...
- Schematic: 0 pin-to-pin conflicts, 0 multiple-net-name shorts (after fixing 3 real bugs
    ...
    project (FlightEngineer has 701 open ERC/DRC items) — dangling labels on intentionally-simplified
...
- **Control half:** TI MSPM0G3507 [REF-SENSOR-004] (native CAN-FD) + Infineon SLB9670 TPM
    (fleet-standard part, already used on all 8 Wash/TACCO nodes — do not introduce a different
...
- [x] **SoM re-scope — Observer = PHYTEC phyCORE PCM-071 SoM on a trapezoidal carrier —
    ...
    by new generators: `avionics/kicad/Observer/scripts/gen_Jayne_carrier_sch.py` (schematic) and
    ...
    - **§0.x OPEN (verify before fab):** regulator feedback-divider/inductor values + 2V5 LDO
      ...
      `avionics/kicad/Observer/JAYNE_SOM_NETMAP.md` are the references.
...
- [x] **FLEET-WIDE ISOW1044BDFMR footprint audit (flight-hardware error, 2026-07-12).** TI
    ...
    "DFM/20 PINS"), NOT the **"SOIC-16W"** that Wash.md, TACCO.md, Observer.md, README.md, and prior
    TODO text all state. Any KiCad footprint using a 16-pad `SOIC-16W` land for U4 (Wash, TACCO,
    Jayne) is WRONG and will not match the 20-pad part. **RESOLVED 2026-07-26.** Observer's was
    already correct (handled by the carrier rebuild). Wash and TACCO's `.kicad_sch` symbols were
    ...
    zero ERC regression (Wash 48 / TACCO 234 violations, unchanged from baseline). Doc text in
    Wash.md/TACCO.md/README.md corrected 2026-07-12; Observer.md corrected same date.
...
- [x] **EMI hardening gap — RESOLVED 2026-07-03.** Observer now matches the Wash/TACCO Rev R
...
- [x] **Create `avionics/kicad/Observer/kicads/Observer.kicad_pcb`** — DONE 2026-07-03 via
    `avionics/kicad/Observer/scripts/gen_Jayne_pcb.py`, through several passes: first-pass double-sided
    ...
    established FlightEngineer/Wash pattern (script places footprints + nets, final routing and
