...
| Capacity | 4000 mAh |
| Mass | 750 g |
| Typical dimensions (Tattu R-Line / Gens Ace 4000 mAh 6S) | 142 × 50 × 38 mm (L × W × H) |
...
For a 610 mm (24") fuselage: **target CG = 280 mm from nose**; acceptable range
237 – 317 mm.
...
| Component group | Mass (g) | Est. station from nose (mm) | Moment (g·mm) |
|---|---|---|---|
| 4× EDFs + nacelle mounts | 380 | 510 | 193 800 |
...
| Airframe (shell + foam fill + hardware) | 600 | 290 | 174 000 |
| Wiring harness + misc | 280 | 295 | 82 600 |
| **Non-battery subtotal** | **2 258** | — | **779 180** |

Non-battery CG = 779 180 / 2 258 = **345 mm from nose** (57 % of fuselage — tail-heavy).

### 2.3 Required Battery Station

To achieve a 280 mm total CG with the battery installed:
...
750 × X_batt = 63 230
X_batt = 84 mm from nose
```

**The battery centroid must be at approximately 84 mm from the nose** — this places
it in the forward section of the head/bridge area of the Serenity model.

### 2.4 Battery Bay Location

The 84 mm station is inside the head (bridge/cockpit) section of the Serenity hull.
The cargo section begins at approximately 100 mm from the nose on the 24" build.
The battery tray therefore spans stations **~60 – 130 mm from the nose**, sitting
beneath the bridge deck of the head section.
...
> **Adjustability:** The tray rail system provides ±20 mm of fore-aft adjustment.
...
| Parameter | Value |
|---|---|
| Interior cavity | 148 × 56 × 44 mm (L × W × H) — battery + 3 mm foam each side |
| Wall thickness | 3.0 mm CF-PETG (structural) |
| Floor thickness | 4.0 mm CF-PETG (load-bearing; battery rests on floor) |
| Rail length | 70 mm (±20 mm travel; locks at 10 mm increments via M3 detent screws) |
| Mounting rail interface | Two 6 × 3 mm CF flat bar slots (mates with CF-BAR-6X3 keel bar) |
| Strap anchor slots | 4× 4 mm wide × 20 mm long through-slots (2 per silicone strap) |
| Anti-vibration lining | 3 mm closed-cell silicone foam bonded to tray floor and side walls |
| Connector exit | Rear wall slot 14 × 10 mm — battery XT60 and balance lead pass through |
| Mass (printed, CF-PETG) | ~22 g |
...
A single 160 × 65 mm belly panel in the head section provides battery access.

| Parameter | Value |
|---|---|
| Material | 2.0 mm PETG (non-structural panel, not CF-PETG) |
| Fasteners | 2× M3 × 8 mm stainless button-head screws; coin-slot drive (no tool needed) |
| Hinge type | None — full removal; panel sits in a 1.5 mm rebate that locates it flush with the hull skin |
| Seal | 2 mm closed-cell EPDM foam tape in rebate (dust seal; not waterproof) |
| Panel mass | ~6 g |
...
When the battery is removed the aircraft CG shifts from 280 mm to 345 mm from the
nose (tail-heavy, but inert). The landing legs (PETG-printed, 4-point contact) are
...
along the CF keel bar for ~80 mm, and enters the Kaylee enclosure through the M16
EMC cable gland at J_BATT. Maximum lead length from battery terminal to Kaylee
enclosure: **160 mm** (keeps cable inductance below 150 nH, well within EMI budget).
