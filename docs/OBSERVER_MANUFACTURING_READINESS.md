# Observer — Manufacturing-Readiness Gap Analysis
...
- **Rev B (2026-07-06):** Added the recommended resolution — mount the AM62A on a vendor
  **system-on-module (SoM)** and rebuild Observer as a **carrier board**, which eliminates the
...
Observer is a **partial architectural sketch, not a fabricable board.** The peripheral/glue
...
## 1.1 Recommended resolution — put the AM62A on a SoM; make Observer a carrier

The root of every blocker is that Observer tries to host a **raw Linux applications SoC** (AM62A7)
...
This **eliminates G1, G2, G4 and most of G6** — they move onto the module — and drops Observer from
...
Industrial** compute modules on capes; raw-SoC Observer was the outlier.
...
Observer needs are native to the part. **Module + form factor LOCKED (2026-07-11, user):** no AM62A SoM fits a 1.0 in-wide board
(narrowest ~30 mm OSM; mainstream 40 mm), so Observer becomes a **trapezoidal carrier** conforming
...
Observer becomes a **carrier** for an AM62A SoM. What lives where:
...
**On the Observer carrier (mostly parts already footprinted — ordinary ~4-layer routing):**
...
3. **Connector stack height + module footprint** fit the nose pod's Z-budget and Observer's
...
**verified pinout already in this repo** (reused from `gen_cape_a2*.py` / existing Observer files) —
only the **SoM-side pad numbers** are datasheet-gated. The generator (a `gen_vera_sch.py`, in
the COMMO pattern) can wire all of this the moment the phyCORE-AM62A pad map is available.
...
- **Populate exactly one per install** (the other is DNP). Both share identical nets, so the
  ...
  no-load.
- This keeps **one board design** for both installs (per the standing Observer principle) and needs
...
- **CMC1–5 (SRF2012-100Y)** currently use `Inductor_SMD:L_Taiyo-Yuden_NR-20xx` — a 2-pin power
  ...
  the `Bourns_SRF2012` (4-pad) footprint used on Wash/TACCO/Observer elsewhere.
...
schematic-first (as with COMMO) once the module + connector P/N are chosen.
