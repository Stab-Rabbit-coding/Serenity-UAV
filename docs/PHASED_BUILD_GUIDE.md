...
| Parameter | Value |
| --- | --- |
| Hull length | 18.00 in (457.2 mm) — canonical 269 ft |
| Beam (nacelle tip-to-tip) | 11.375 in (288.9 mm) — canonical 170 ft |
| Propulsion | **2× (2× Changesun XRP 3660-2700KV 80mm 6S, tandem series) per nacelle** + 1× XFLY X4 PRO 40mm 4S fuselage |
| Nacelle pod | 3.68 in (93.5 mm) OD × **9.06 in (230 mm)** length (tandem dual-EDF) · 3.27 in (83 mm) ID |
...
| CN node hardware | **PocketBeagle 2 Industrial (AM6254)** + TACCO 55×35mm — SiK 915MHz, LoRa RFM95W 915MHz, TI WL1837MOD WiFi/BT, XCVR-49MHZ sub-module, MIL-STD-1553, CAN FD, RS-485, Ethernet; **SLB9670 TPM 2.0**; ATF16V8BQL CPLD write-blocker (log μSD) · DK 2820-100003007-ND |
...
| Cape variant layout | **v2 · v2 · v2 · v2 (nose → tail, Rev Q):** All 8 positions use Wash / TACCO (EMI-hardened, 5 kV isolated CAN FD / RS-485 / Ethernet transceivers). Single-SKU procurement; Cape-A-1 / Cape-B-1 / XCVR-49MHZ-1 archived as of Rev Q. |
...
- Step 4: **Install the final PTFE conduit tubes sized for the final cables.** The conduit tube ID is 3 mm; use pull strings from Phase 1. Route cables phase by phase as they're needed.

- Step 5: **Budget EDFs first, XRP EDFs last.** First flight uses affordable motors. Upgrade the nacelle pods and ESCs together in Phase 7 after the airframe and avionics are proven.
...
| ----------- | -------------------- | ------------------------------------ | ------------------------------------------------- |
| Keel        | Dremel cut-off disc  | 6×3mm flat bar → 17.99 in (457.2 mm)  | Mark datums at 3.58, 6.50, 9.88, 12.60, 15.28 in (91, 165, 251, 320, 388 mm) from nose |
| Spars       | Pipe cutter / Dremel | 12mm OD tube → 2× 11.81 in (300 mm)  | Sand inner (spar end) to 8 mm dia × 25 mm tenon   |
...
| Panel           | Station in (mm)        | Bay                          | Void size (in / mm)          | Closure            |
| --------------- | ---------------------- | ---------------------------- | ---------------------------- | ------------------ |
| A — Nose        | 0–3.58 in (0–91 mm)   | CN1+FC1 (CN lower, FC upper) | 3.82×2.48×3.39 in (97×63×86 mm) EPS | Bayonet PETG frame |
| B — Dorsal Fwd  | 3.58–6.50 in (91–165 mm) | CN2+FC2 (CN lower, FC upper) | 3.82×2.48×2.91 in (97×63×74 mm) EPS | 4× M2.5 screws     |
| C — Cargo Belly | 6.30–9.88 in (160–251 mm) | cargo                     | 2.76×1.89×3.58 in (70×48×91 mm) EPS | Hinge PETG frame   |
| D — Dorsal Aft  | 9.88–12.60 in (251–320 mm) | CN3+FC3 (CN lower, FC upper) | 3.82×2.48×2.72 in (97×63×69 mm) EPS | 4× N42 magnets     |
...
- Step 2: Solder 14AWG main leads: PDB to ESC-L (port, ~145mm run), ESC-R (stbd, ~155mm), ESC-Fwd (fuselage, ~290mm).
...
- Step 6: Pull 14AWG motor phase leads through PWR conduit to ESC terminals. Solder — check rotation marking before final solder order (swap any two leads to reverse direction).
...
- Step 15: Pull CAN-FD conduit to Bay A. Daisy-chain:**120Ω soldered termination** → CN1 CAN in → CN1 CAN out → FC1 CAN in → FC1 CAN out → exit Bay A toward Bay B via CAN conduit.

- Step 16: Pull RS-485 conduit to Bay A. Daisy-chain CN1 → FC1 JST-GH RS-485 ports; cable exits toward Bay B.
...
- Step 24: Continue CAN-FD daisy-chain from Bay A into Bay B: CN2 CAN in → CN2 CAN out → FC2 CAN in → FC2 CAN out +**temporary 120Ω termination** (Phase 3 far-end — remove in Phase 4 when full bus is populated).

- Step 25: Continue RS-485 daisy-chain from Bay A into Bay B: CN2 → FC2 ports.
...
- Step 33: Configure forensic log mount on CN1 and CN2:
...
- Step 34: OS image: BeagleBoard.org Debian 12 "Bookworm" for PocketBeagle 2 Industrial (AM6254 target). Flash to 64GB eMMC via USB-C using BeagleBoard flasher image — no microSD needed. Boot from eMMC, SSH in, update:
...
- Step 35: Enable CAN FD interfaces on each node:
...
- Step 38: Calibrate ESCs (no loads connected):
- Arm sequence per BLHeli32 (full throttle power-on → drop to zero)
- Verify all 3 ESCs calibrated and armed

- Step 39: Calibrate nacelle tilt servos: command 0° → verify nacelle vertical ±0.5°; command 90° → verify horizontal ±0.5°. Adjust servo horn if needed.

- Step 40: Calibrate fuselage nozzle servo endpoints:
- 1.0ms PWM → nozzle open (42mm exit) — lock clevis
- 2.0ms PWM → nozzle closed (36mm exit) — verify

### Ground Tests — Verify Before Any Flight

- Step 41: Static CG: target **7.48 in (190 mm) from nose**. Slide battery position on rail to achieve. Measure with wing tips level on known-flat surface.
...
- Step 44: Tethered thrust test (tie-down hook rated ≥10kg on keel):
...
- Step 45: Node failover test: with FC master node active, kill FC master power → verify standby FC node assumes authority within 100ms (CAN FD arbitration), altitude hold maintained on tether.

- Step 46: Nav lights: all 6 positions cycle through ICAO states — RED port, GREEN stbd, WHITE steady tail, WHITE strobe belly.

- Step 47: GPS lock: HDOP ≤1.5 with both nacelle EDFs running at 30%. FC1 and FC2 each report independent GPS fix — cross-check positions agree within 2m.
...
- Step 2: Free hover 1m AGL: stability check, ±10° roll/pitch/yaw authority, altitude hold ±0.3m — ≤60s

- Step 3: Free hover 3m AGL: yaw 360° both directions

- Step 4: Nacelle transition: climb to ≥8m (26ft) AGL, command gradual nacelle sweep 90°→0° → verify altitude hold ±1.5m during transition

- Step 5: Forward flight circuit: one lap of flying area at ≤10m AGL, transition back to hover, land

- Step 6: Record telemetry log. Verify flight data written to CN1 and CN2 log μSDs (file size increases on both cards).
...
- Step 25: RS-485 multidrop: all 8 nodes active on bus. Permanent termination at CN1 (Bay A, bus start) and FC4 (Bay E, bus end) — no additional termination changes needed.
...
- Step 5: Configure OA fusion across both arrays. GPS position is broadcast on CAN FD by the elected FC master; any FC node can run waypoint planning and OA fusion.
...
- Step 2: Disconnect all 4 budget EDF motor leads at hull exit (FWD and AFT).

- Step 3: Unscrew 4× M2.5 from AFT EDF. Slide AFT EDF out of pod aft face.

- Step 4: Unscrew 4× M2.5 from FWD EDF. Slide FWD EDF out of pod intake.

## 5. The `nacelle_pod_dual_80mm.stl` pod remains on the pivot bracket — do not remove the pod

- Step 6: Slide XRP FWD EDF into intake (housing OD 83mm → 83mm ID pod = slip fit). Torque 4× M2.5 to 0.3 N·m. Route 14AWG phase leads through FWD channel, label "FWD".

- Step 7: Slide XRP AFT EDF in from aft face. Mount as above. Route 14AWG leads through AFT channel, label "AFT".

- Step 8: Verify blade tips clear pod wall at 0°, 45°, 90°, 115° nacelle angles — rotate by hand.

- Step 9: Reinstall nacelle tip cap.

### ESC Replacement Sequence

- Step 10: Remove 4× budget ESCs from Bay C/D (one per nacelle EDF).

- Step 11: Mount 4× Hobbywing Platinum PRO V4 120A ESCs:
...
- Step 14: Update CG: XRP quad vs budget quad adds significant mass. Recheck CG at 190mm from nose with flight battery. Shift battery on rail as needed.

### Rev L PID Governor Commissioning

- Step 15: Flash Rev L governor firmware to FC1–FC4:
...
- Step 16: Bench calibration (nacelle on thrust stand with load cell, bench power supply):
...
- Step 17: Update `governor_config.h` with measured k values. Re-flash to FC1–FC4.

- Step 18: PID step-response verification (bench, EDF in pod, tethered):
...
- Step 19: Fault injection test (bench, tethered):
...
- Step 20: Bench thrust test (tie-down, tethered, all 4 nacelle EDFs running):
...
- Step 21: ESC temperature: Hobbywing 120A ESC junction ≤80°C at sustained full throttle 10s on all 4 nacelle ESCs. Monitor with IR thermometer.

- Step 22: T/W verification:
...
- Step 23: Max payload test (incremental, tethered hover):
...
- Step 24: Full performance flight: forward flight at cruise speed, nacelle transitions, 1-EDF fault injection at altitude (governor maintains flight), cargo delivery, RTH.
...
- Step 2: Print decal sheet on white waterslide decal paper (inkjet or laser per paper type). Seal with clear coat after printing; allow 24h dry.

- Step 3: Apply decals per `diagrams/build_guide_19_decal_placement.svg`:
