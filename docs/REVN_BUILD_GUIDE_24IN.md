...
- Step 2: **Install every conduit, standoff, void former, and sensor mount before the foam pour.** Once foam cures these cannot be added.

- Step 3: **The foam pour is the point of no return.** Do a full dry-fit with all wiring and components before mixing.

- Step 4: **Do NOT foam the nacelle interior.** Nacelles are CF-PETG structural shells — foam in the bore traps EDF heat and ruins the press-fit.

- Step 5: **Verify EDF rotation direction before nacelle installation.** Port EDFs must rotate CW from the intake end; starboard CCW. Swap any two motor leads to reverse if wrong. Once nacelle is sealed and fiberglassed, this is a full disassembly.

- Step 6: **Gear-mesh the nozzle linkage before bonding the tilt bracket.** Check nozzle opens smoothly through the full 0°–90° sweep before epoxying the sector gear.

- Step 7: **FAA registration on airframe before first untethered flight.** Replace placeholder with your issued number.

- Step 8: **TPM 2.0 provisioning is non-reversible.** Provision all 8 nodes before first flight.
...
- Step 6: **Cure 2h minimum** before proceeding.
...
- Step 2: **Install nozzle inner ring** (rack, R=28mm) inside the base ring — this is the driven element.

- Step 3: **Bend piano wire link ring** (0.8mm wire): one closed ring that passes through all 8 petal link holes and loops around the inner ring posts. The ring translates radial rotation of the inner ring into petal rotation.

- Step 4: **Install 8 petals** on 3mm hinge pins. Hinge pins press into base ring lugs. Each petal sits on one hinge pin; the piano wire link passes through the petal's bottom lug.

- Step 5: **Dry-test:** manually rotate the inner ring through its travel. Petals should open smoothly from closed (hull-matched position) to ~75° open. Check for binding.

- Step 6: **Install WS2812B LED ring** at duct exit lip inside the base ring. Three-wire lead routes through hub bore to fuselage LED driver (Cape-B).
...
- Step 2: Mount drive pinion on nacelle outer shell at pivot axis. Verify mesh with sector gear; set backlash 0.1–0.2mm.

- Step 3: Install bevel gear pair in nacelle body. Input shaft = nacelle-body-axis; output = longitudinal shaft toward nozzle.

- Step 4: Thread longitudinal 2mm steel shaft through nacelle wall channel toward nozzle end.

- Step 5: Mount crown pinion on longitudinal shaft at nozzle end. Verify mesh with nozzle inner ring rack; set backlash 0.1–0.2mm.

- Step 6: **Sweep test:** manually rotate nacelle from 0° to 90°. Nozzle inner ring should rotate ~71°; petals should open from fully closed to fully open.

- Step 7: Verify nozzle inner ring hard stop prevents over-drive at nacelle angles 90°–120°.

- Step 8: Confirm petal closed position matches nacelle hull profile at 0°.
...
- Step 2: **Insert 4mm CF pivot rod** through the wing spar pocket and through the pivot housing bearings. The pivot rod is fixed to the fuselage; the nacelle rotates around it on the bearings.

- Step 3: **Slide nacelle pivot housing** onto pivot rod. Verify nacelle rotates freely with <0.5 mm axial play. Add thrust washer (stainless 4×8×0.5mm) if axial play exceeds 1mm.
...
- Step 5: **Fill any gap between shoulder flange and hull exterior** with epoxy fillet. Smooth with gloved finger. Cure 2 h.
...
- Step 5: **Install 80A ESC** in Panel F bay. Bond ESC to bay floor with double-sided foam tape + cable tie.

- Step 6: **Cure 2 h minimum before applying thrust.**
...
- Step 2: **Install 8 rear nozzle petals** on 3mm hinge pins. Piano wire link ring passes through all petal bottom lugs.

- Step 3: **Install rear nozzle servo (SG90 class)** inside Panel F section. Piano wire pushrod connects servo arm to nozzle inner ring.

- Step 4: **Calibrate:** servo 0° = petals closed (hull-matched bell profile); servo 90° = petals fully open (full burn).

- Step 5: **Install WS2812B LED ring** at rear duct exit lip.
...
- Step 2: Seat Cape-A (FC node) on 20mm inter-cape standoffs above Cape-B. 4 screws.

- Step 3: Plug 72-pin expansion connector between PB2-I and Cape.

- Step 4: Connect CAN FD, RS-485, 1553, ETH chain connectors per bus harness.

- Step 5: Connect ESC DSHOT signal leads to Cape-A PRU-ICSS header.

- Step 6: Connect servo signal leads (2× tilt servos + 1× rear nozzle servo) to Cape-A servo rail.

- Step 7: Connect GPS patch antenna coax to Cape-A GPS header. Mount GPS patch antenna on hull dorsal surface at nearest bay location.
...
- Step 2: Flash Rev N PID governor firmware binary to M4F coprocessor on each FC node.

- Step 3: Upload via CN1 MAVLink → ETH ring → TPM-verified flash on FC1, FC2.

- Step 4: Verify `GOV_STATE = NORMAL` on all active ESCs via MAVLink.
...
- Step 2: Bring up on throttle to 25% — verify hover, check all 5 EDF sounds.

- Step 3: Increase to 60% — verify stable hover and tilt response.

- Step 4: Verify nacelle nozzle state at commanded hover angle.

- Step 5: **Only after 3 successful tethered flights:** proceed to free hover.
...
- Step 2: **NAV lights.** ICAO Annex 2: port = red, stbd = green, aft/top = white strobe. Wire to Cape-B CN4 GPIO.

- Step 3: **FAA registration.** Display on hull exterior per 14 CFR 47 — replace N00000 on decal sheet with your issued number.

- Step 4: **Firmware security provisioning.** Program Cape-B CPLD write-blocker (ATF16V8BQL). Provision SLB9670 TPM 2.0 endorsement keys on all 8 nodes. Keys cannot be regenerated without physical node replacement.

- Step 5: **Weight and balance.** Measure actual AUW with battery. CG should be at 40–45% of hull length from nose. Adjust battery position in cargo bay for trim if needed.

- Step 6: **Documentation.** Log all configuration in `governor_config.h`. Archive MAVLink parameter file. Photograph completed build for insurance / forensic baseline.
